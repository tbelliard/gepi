...
if (isset($var) AND is_numeric($var)) {
	...
	$query_d = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom, nom_long, subdivision_type, subdivision FROM edt_gr_nom WHERE id = '".$var."'");
...
if ($var2 == "changer_nom") {
	...
	$query_e = mysqli_query($GLOBALS["___mysqli_ston"], $sql_e) OR trigger_error('Impossible de récupérer la liste des élèves', E_USER_ERROR);

	while($rep = mysqli_fetch_array($query_e)){

		// On récupère alors la classe
		$query_c = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT classe FROM j_eleves_classes jec, classes c
	...
	$sql_p = "SELECT login, nom, prenom FROM edt_gr_profs egp, utilisateurs u
										...
										AND id_gr_nom = '".$var."'
										ORDER BY nom, prenom";
	$query_p = mysqli_query($GLOBALS["___mysqli_ston"], $sql_p) OR trigger_error("Impossible de récupérer la liste des professeurs de ce groupe : ", E_USER_ERROR);
...
if ($action == "modifier_gr") {
	...
	$query_m = mysqli_query($GLOBALS["___mysqli_ston"], $sql_m) OR trigger_error('Impossible de mettre à jour ce groupe '.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)), E_USER_ERROR);
	if ($query_m) {
		// On ferme la fenêtre
