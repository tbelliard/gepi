...
// This position can be useful because we don't know in advance how $Value will be replaced.
	...
	if ($Loc->Ope) {
		foreach ($Loc->OpeAct as $i=>$ope) {
			...
			case  4: if (mb_strlen(''.$CurrVal)>$Loc->OpePrm[$i]) $CurrVal = mb_substr(''.$CurrVal,0,$Loc->OpePrm[$i],'UTF-8').$Loc->OpeEnd; break;
...
function meth_Merge_BlockSections(&$Txt,&$LocR,&$Src,&$RecSpe) {
  ...
  // Plug-ins
	...
	if ($LocR->P1) $LocR->FOStop = $LocR->PosBeg + mb_strlen($BlockRes) -1;
...
function meth_Merge_SectionNormal(&$BDef,&$Src) {
	...
	if (isset($BDef->AutoSub)) {
		for ($i=1;$i<=$BDef->AutoSub;$i++) {
			...
			if ($col_opt) $col = substr($col,1,mb_strlen($col)-2);
			if ($col==='') {
...
function meth_Misc_UserFctCheck(&$FctInfo,$FctCat,&$FctObj,&$ErrMsg,$FctCheck=false) {
	...
	if ($FctStr[0]==='~') {
		...
		if ( $FctCheck && ($this->FctPrefix!=='') && (strncmp($this->FctPrefix,$FctStr,mb_strlen($this->FctPrefix))!==0) ) {
...
// att=div#class ; att=((div))#class ; att=+((div))#class
	...
	foreach ($TagLst as $i=>$Tag) {
		$LevelStop = false;
		while ((mb_strlen($Tag)>1) and (substr($Tag,0,1)==='(') and (substr($Tag,-1,1)===')')) {
			if ($LevelStop===false) $LevelStop = 0;
			$LevelStop++;
			$Tag = trim(substr($Tag,1,mb_strlen($Tag)-2));
		}
...
static function f_Xml_AttMove(&$Txt, &$Loc, $AttDelim=false) {
  ...
  if ($AttDelim===false) $AttDelim = '"';
	...
	if ($Loc->AttBeg===false) {
		...
		$Ins2 = $AttDelim;
		$Loc->AttBeg = $InsPos + 1;
		$Loc->AttValBeg = $InsPos + mb_strlen($Ins1) - 1;
	} else {
	...
	if ($InsPos===false) {
		...
		$InsTxt = $Ins1.'[]'.$Ins2;
		$InsLen = mb_strlen($InsTxt);
		$PosBeg = $InsPos + mb_strlen($Ins1);
		$PosEnd = $PosBeg + 1;
...
*/
	...
	$PosEnd = false;
	$TagL = mb_strlen($Tag);
	$TagClosingL = mb_strlen($TagClosing);
	$RightLevel = 0;
