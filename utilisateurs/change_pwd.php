...
if (isset($_POST['valid']) and ($_POST['valid'] == "yes")) {
	...
	if(($mdp_INE=='y')&&($user_statut=='eleve')&&($ine_password!="")) {
		$auth_mode = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT auth_mode FROM utilisateurs WHERE login = '".$user_login."'"), 0);
		...
		if ($_SESSION['statut'] == 'administrateur') {
			$reg_data = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE utilisateurs SET change_mdp = 'y' WHERE login='".$user_login."'");
		}
...
if (isset($user_login) and ($user_login!='')) {
    $call_user_info = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom,prenom,statut,auth_mode FROM utilisateurs WHERE login='".$user_login."'");
