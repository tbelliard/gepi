...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	...
	if(isset($definir_resp)){
		if(!isset($valider_choix_resp)){
			...
			echo "<p>Choix du responsable légal <b>$definir_resp</b> pour <b>".casse_prenom($eleve_prenom)." ".my_strtoupper($eleve_nom)."</b></p>\n";
			...
			if ($nombreligne != 0) {
				...
				while($lig_resp=mysql_fetch_object($call_resp)){
					...
					echo "<td><a href='../responsables/modify_resp.php?pers_id=$lig_resp->pers_id&amp;quitter_la_page=y' target='_blank'>".my_strtoupper($lig_resp->nom)." ".casse_prenom($lig_resp->prenom)."</a></td>\n";
					echo "<td>";
	...
	if(isset($definir_etab)){
		if(!isset($valider_choix_etab)){
			...
			*/
			...
			echo "<p>Choix de l'établissement d'origine pour <b>".casse_prenom($eleve_prenom)." ".my_strtoupper($eleve_nom)."</b></p>\n";
	...
	if ($nombreligne != 0) {
		...
		if("$eleve_no_resp1"=="0"){
			...
			if(mysql_num_rows($res_resp)==0){
				...
				echo "<td>";
				if($_SESSION['statut']=="professeur") {
					echo casse_prenom($lig_resp->prenom)." ".my_strtoupper($lig_resp->nom);
				}
				else{
		...
		if("$eleve_no_resp2"=="0"){
			...
			if(mysql_num_rows($res_resp)==0){
				...
				if($_SESSION['statut']=="professeur") {
					echo "<td>".casse_prenom($lig_resp->prenom)." ".my_strtoupper($lig_resp->nom)."</td>\n";
				}
				else{
					...
					echo "<td><a href='../responsables/modify_resp.php?pers_id=$eleve_no_resp2&amp;quitter_la_page=y' onclick=\"return confirm_abandon (this, change, '$themessage');\" target='_blank'>".casse_prenom($lig_resp->prenom)." ".my_strtoupper($lig_resp->nom)."</a></td>\n";
