...
if (isset($_POST['is_posted']) and ($_POST['is_posted'] == "1")) {
	...
	if (($continue == 'yes') and (isset($reg_login))) {
		...
		if (preg_match("/^[a-zA-Z_]{1}[a-zA-Z0-9_.]{0,".($longmax_login_eleve-1)."}$/", $reg_login)) {
			...
			if ($ok == 'yes') {
				...
				if ($count == "0") {
					...
					$reg_data1 = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO eleves SET
						no_gep = '".$reg_no_nat."',
						nom='".$reg_nom."',
						prenom='".$reg_prenom."',
						email='".$reg_email ."',
					...
					if($reg_resp1!=""){
						...
						$test_resp1=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
						if(mysqli_num_rows($test_resp1)>0){
							...
							$test_resp1b=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
							if(mysqli_num_rows($test_resp1b)==1){
								...
								$test_resp1c=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
							...
							if($temoin_maj_resp==""){
								...
								$reg_data2b=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
								if(!$reg_data2b){
									$msg.="Erreur lors de la mise à jour du responsable $reg_resp1 en responsable légal n°1.<br />\n";
								}
							}
					...
					if ($reg_no_gep != '') {
						$call_test = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM j_eleves_etablissements WHERE id_eleve = '$reg_no_gep';");
						$count2 = mysqli_num_rows($call_test);
						if ($count2 == "0") {
							if ($reg_etab != "(vide)") {
								$reg_data2 = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_eleves_etablissements VALUES ('$reg_no_gep','$reg_etab');");
							}
						} else {
							if ($reg_etab != "(vide)") {
								$reg_data2 = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE j_eleves_etablissements SET id_etablissement = '$reg_etab' WHERE id_eleve='$reg_no_gep';");
							} else {
								//$reg_data2 = mysql_query("DELETE FROM j_eleves_etablissements WHERE id_eleve='$reg_login'");
								$reg_data2 = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM j_eleves_etablissements WHERE id_eleve='$reg_no_gep';");
							}
						}
					}
		...
		$reg_data = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE eleves SET no_gep = '$reg_no_nat', nom='$reg_nom',prenom='$reg_prenom',email='$reg_email',sexe='$reg_sexe',naissance='".$reg_naissance."', ereno='".$reg_resp1."', elenoet = '".$reg_no_gep."' WHERE login='".$eleve_login."'");
		if (!$reg_data) {
			$msg = "Erreur lors de l'enregistrement des données";
		} else {
			// On met à jour la table utilisateurs si un compte existe pour cet élève
			$test_login = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(login) FROM utilisateurs WHERE login = '".$eleve_login ."'"), 0);
			if ($test_login > 0) {
				$res = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE utilisateurs SET nom='".$reg_nom."', prenom='".$reg_prenom."', email='".$reg_email."' WHERE login = '".$eleve_login."'");
		...
		if ($reg_no_gep != '') {
			...
			if ($count == "0") {
				if ($reg_etab != "(vide)") {
					$reg_data = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_eleves_etablissements VALUES ('$reg_no_gep','$reg_etab');");
				}
			} else {
				if ($reg_etab != "(vide)") {
					$reg_data = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE j_eleves_etablissements SET id_etablissement = '$reg_etab' WHERE id_eleve='$reg_no_gep';");
				} else {
					$reg_data = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM j_eleves_etablissements WHERE id_eleve='$reg_no_gep';");
				}
			}
		}
...
if (isset($eleve_login)) {
    ...
    $call_etab = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT e.* FROM etablissements e, j_eleves_etablissements j WHERE (j.id_eleve='$eleve_login' and e.id = j.id_etablissement)");
...
<?php
$calldata = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM etablissements ORDER BY id");
