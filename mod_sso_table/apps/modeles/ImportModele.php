...
class ImportModele {
  ...
  function search($nom) {
    ...
    while ($this->row = mysql_fetch_array($this->res, MYSQL_NUM)) {
  ...
  function get_login_sso_table_sso($login) {
    ...
    $this->res = mysql_query($this->req) or die(mysql_error());
    $this->q = mysql_fetch_array($this->res);
    if ($this->q)
      return $this->q[0];
  }
  ...
  function get_nbre_entrees() {
    ...
    $this->res = mysql_query($this->req) or die(mysql_error());
    $resultat = (mysql_fetch_row($this->res));
    return $resultat[0];
  }
  ...
  function get_logins_gepi_table() {
    ...
    $this->res = mysql_query($this->req) or die(mysql_error());
    while ($this->q = mysql_fetch_assoc($this->res)) {
  ...
  public function get_infos_classes() {
    ...
    $this->res = mysql_query($this->sql) or die(mysql_error());
    return $this->res;
  }
