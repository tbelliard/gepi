...
if ((!isset($id_classe)) or ($id_classe=='')) {
	...
	elseif($_SESSION['statut']=='professeur'){
		$call_classes=mysql_query("SELECT DISTINCT c.* FROM classes c, periodes p, j_groupes_classes jgc, j_groupes_professeurs jgp WHERE p.id_classe = c.id AND jgc.id_classe=c.id AND jgp.id_groupe=jgc.id_groupe AND jgp.login='".$_SESSION['login']."' ORDER BY c.classe");
	}
	elseif($_SESSION['statut']=='cpe'){
		$call_classes=mysql_query("SELECT DISTINCT c.* FROM classes c, periodes p, j_eleves_classes jec, j_eleves_cpe jecpe WHERE
...
//***************************************************************************************************
    ...
    while ($i < $nombre_lignes) {
        ...
        if ($current_group2) {
            ...
            echo "<tr><td>" . htmlentities($current_group["matiere"]["nom_complet"]) . "</td>\n";
            $k = '1';
            while ($k < $nb_periode) {
                ...
                $moyenne_classe2_query = mysql_query("SELECT round(avg(note),1) as moyenne FROM matieres_notes WHERE (periode='$k' AND id_groupe='" . $current_group2["id"] . "' AND statut = '')");
