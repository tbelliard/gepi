...
if (!isset($quitter_la_page)) {
	...
	if ($id_class_suiv != 0) {
		echo " <a href='" . $_SERVER['PHP_SELF'] . "?id_classe=$id_class_suiv' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/arrow-right.png' class='icone16' title='Classe suivante' /></a>";
	}
	...
	if ($id_class_suiv != 0) {
		echo " <a href='" . $_SERVER['PHP_SELF'] . "?id_classe=$id_class_suiv&amp;quitter_la_page=y' onclick=\"return confirm_abandon (this, change, '$themessage')\"><img src='../images/icons/arrow-right.png' class='icone16' title='Classe suivante' /></a>";
	}
	echo "</p>\n";
	echo "</form>\n";
...
<p class='small'><b>Remarque&nbsp;:</b> lors du retrait d'un élève de la classe pour une période donnée, celui-ci sera
	retiré de tous les enseignements auxquels il était inscrit pour la période en question.</p>
<?php
...
";
	...
	for ($loop = 0; $loop < count($tab_profsuivi); $loop++) {
		...
		if ($tab_profsuivi[$loop] == $login_pp_unique_actuel) {
			echo " selected='true'";
		}
	...
	for ($loop = 0; $loop < count($tab_cperesp); $loop++) {
		$ind = $loop + 1;
		echo "<option value='$ind'";
		if ($tab_cperesp[$loop] == $login_cpe_unique_actuel) {
			echo " selected='true'";
		}
...
à la classe : " . formate_date($date_fin_periode[$i]) . "\">Retirer de la classe<br />$nom_periode[$i]<br />\n";
	...
	while ($k < $nombreligne) {
		...
		echo "<td style='text-align: center; border: 0;'>I-ext<br /><input type='radio' name='regime_eleve[$k]' value='i-e' ";
		if ($regime == 'i-e') {
			echo " checked";
		}
		echo " onchange='changement()'";
		echo " /></td>\n";
		echo "<td style='text-align: center; border: 0; border-left: 1px solid #AAAAAA;'>Int<br/><input type='radio' name='regime_eleve[$k]' value='int.' ";
		if ($regime == 'int.') {
			echo " checked";
		}
		echo " onchange='changement()'";
		echo " /></td>\n";
		echo "<td style='text-align: center; border: 0; border-left: 1px solid #AAAAAA;'>D/P<br/><input type='radio' name='regime_eleve[$k]' value='d/p' ";
		if ($regime == 'd/p') {
			echo " checked";
		}
		echo " onchange='changement()'";
		echo " /></td>\n";
		echo "<td style='text-align: center; border: 0; border-left: 1px solid #AAAAAA;'>Ext<br/><input type='radio' name='regime_eleve[$k]' value='ext.' ";
		if ($regime == 'ext.') {
			echo " checked";
		}
		//=========================
		...
		for ($loop = 0; $loop < count($tab_cperesp); $loop++) {
			...
			if ($tab_cperesp[$loop] == $eleve_cperesp) {
				echo " selected";
			}
			echo ">" . casse_mot($tab_cpeprenom[$loop], 'majf2') . " " . my_strtoupper($tab_cpenom[$loop]) . "</option>\n";
		}
...
à la classe : " . formate_date($date_fin_periode[$i]) . "\">";
				...
				if (($nb_verif1 == 0) && ($nb_verif2 == 0) && ($nb_verif3 == 0)) {
					...
					$motif_bloquant = "";
					if ($nb_verif1 > 0) {
						$motif_bloquant .= "$nb_verif1 moyenne(s)";
					}
					if ($nb_verif2 > 0) {
						if ($motif_bloquant != "") {
							$motif_bloquant .= ", ";
