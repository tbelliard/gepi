...
if (isset($_POST['is_posted'])) {
	...
	while ($per < $max_periode) {
		...
		while ($nbc < $nb_classe) {
			...
			$query_per = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT p.num_periode FROM classes c, periodes p WHERE (p.id_classe = c.id  and c.id = '".$id_classe."')");
			...
			if ($nb_periode == $per) {
				...
				if (isset($_POST[$temp])) {
					...
					if ($_POST[$temp2] != '') {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET suivi_par='".$_POST[$temp2]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - reg_suivi_par".$per." : ".$_POST[$temp2]."</br>";
					...
					if ($_POST[$temp2] != '') {
						...
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET formule='".html_entity_decode($_POST[$temp2])."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - reg_formule".$per." : ".$_POST[$temp2]."</br>";
					...
					if (isset($_POST['nb_'.$per.'_reg_format'])) {
						...
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET format_nom='".$tab[2]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
//                        echo "classe : ".$id_classe." - ".$_POST['nb_'.$per.'_reg_format']."</br>";
					}
					if (isset($_POST['display_rang_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_rang='".$_POST['display_rang_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					//====================================
					// AJOUT: boireaus
					if (isset($_POST['display_address_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_address='".$_POST['display_address_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					if (isset($_POST['display_coef_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_coef='".$_POST['display_coef_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					if (isset($_POST['display_nbdev_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_nbdev='".$_POST['display_nbdev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}


					if (isset($_POST['display_moy_gen_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_moy_gen='".$_POST['display_moy_gen_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}


					if (isset($_POST['display_mat_cat_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET display_mat_cat='".$_POST['display_mat_cat_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if ((isset($_POST['modele_bulletin_'.$per])) AND ($_POST['modele_bulletin_'.$per]!=0)) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET modele_bulletin_pdf='".$_POST['modele_bulletin_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_nomdev_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_nomdev='".$_POST['rn_nomdev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_toutcoefdev_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_toutcoefdev='".$_POST['rn_toutcoefdev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_coefdev_si_diff_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_coefdev_si_diff='".$_POST['rn_coefdev_si_diff_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_datedev_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_datedev='".$_POST['rn_datedev_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_abs_2_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_abs_2='".$_POST['rn_abs_2_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_sign_chefetab_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_sign_chefetab='".$_POST['rn_sign_chefetab_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_sign_pp_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_sign_pp='".$_POST['rn_sign_pp_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}

					if (isset($_POST['rn_sign_resp_'.$per])) {
						$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_sign_resp='".$_POST['rn_sign_resp_'.$per]."' where id='".$id_classe."'");
						if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
					}
					...
					if($_POST['rn_sign_nblig_'.$per]!="") {
						...
						if (isset($_POST['rn_sign_nblig_'.$per])) {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_sign_nblig='".$_POST['rn_sign_nblig_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					if (isset($_POST['rn_formule_'.$per])) {
						if ($_POST['rn_formule_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET rn_formule='".$_POST['rn_formule_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					if (isset($_POST['ects_fonction_signataire_attestation_'.$per])) {
						if ($_POST['ects_fonction_signataire_attestation_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET ects_fonction_signataire_attestation='".$_POST['ects_fonction_signataire_attestation_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					if (isset($_POST['ects_type_formation_'.$per])) {
						if ($_POST['ects_type_formation_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET ects_type_formation='".$_POST['ects_type_formation_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_parcours_'.$per])) {
						if ($_POST['ects_parcours_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET ects_parcours='".$_POST['ects_parcours_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_domaines_etude_'.$per])) {
						if ($_POST['ects_domaines_etude_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET ects_domaines_etude='".$_POST['ects_domaines_etude_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
                    ...
                    if (isset($_POST['ects_code_parcours_'.$per])) {
						if ($_POST['ects_code_parcours_'.$per]!='') {
							$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE classes SET ects_code_parcours='".$_POST['ects_code_parcours_'.$per]."' where id='".$id_classe."'");
							if (!$register) $reg_ok = 'no'; else $reg_ok = 'yes' ;
						}
					}
					...
					while ($row = mysqli_fetch_array($get_cat,  MYSQLI_ASSOC)) {
						...
						if($reg_priority!='') {
							...
							$test = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "select count(classe_id) FROM j_matieres_categories_classes WHERE (categorie_id = '" . $row["id"] . "' and classe_id = '" . $id_classe . "')"), 0);
							if ($test == 0) {
								...
								$res = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE j_matieres_categories_classes SET priority = '" . $reg_priority . "', affiche_moyenne = '" . $reg_aff_moyenne . "' WHERE (classe_id = '" . $id_classe . "' and categorie_id = '" . $row["id"] . "')");
							}
							if (!$res) {
					...
					if((isset($_POST['change_coef']))&&($_POST['change_coef']=='y')) {
						if((isset($_POST['coef_enseignements']))&&($_POST['coef_enseignements']!="")) {
							...
							if($coef_enseignements!="") {
								...
								$update_coef=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
								if(!$update_coef) {
							...
							if(mysqli_num_rows($verif)==0) {
								...
								if($professeur_nouvel_enseignement!="") {
									...
									if(mysqli_num_rows($verif)==0) {
										...
										$res_ordre_matieres=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
					...
					if((isset($_POST['change_visibilite']))&&(isset($_POST['matiere_modif_visibilite_enseignement']))&&($_POST['matiere_modif_visibilite_enseignement']!="")) {
						...
						//echo "$sql<br />";
						$res_grp_vis=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
						...
						if($modif_enseignement_visibilite2!="") {
							...
							if(isset($tmp_tab_vis[1])) {
								if($matiere_modif_coef=='___Tous_les_enseignements___') {
									...
									$res_modif_coef=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
									if(!$res_modif_coef) {
									...
									*/
									...
									$res_grp_modif_coef=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
