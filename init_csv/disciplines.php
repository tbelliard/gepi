...
if (!isset($_POST["action"])) {
	...
	if ($_POST['action'] == "save_data") {
		...
		while ($lig=mysql_fetch_object($res_temp)) {
			...
			if (strlen($reg_nom_court) > 50) $reg_nom_court = mb_substr($reg_nom_court, 0, 50);
			...
			if (strlen($reg_nom_long) > 200) $reg_nom_long = mb_substr($reg_nom_long, 0, 200);
				...
				//=========================
					...
					while (!feof($fp)) {
						...
						if(trim($ligne)!="") {
							...
							if (strlen($tabligne[1]) > 200) $tabligne[1] = mb_substr($tabligne[1], 0, 200);
