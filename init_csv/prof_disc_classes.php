	...
	if ($_POST['action'] == "save_data") {
		...
		while ($lig=mysql_fetch_object($res_tempo4)) {
			...
			*/
			...
			if (strlen($reg_prof) > 50) $reg_prof = mb_substr($reg_prof, 0, 50);

			$reg_matiere = preg_replace("/[^A-Za-z0-9.\-]/","",trim(strtoupper($reg_matiere)));
			if (strlen($reg_matiere) > 50) $reg_matiere = mb_substr($reg_matiere, 0, 50);

			$reg_classes = preg_replace("/[^A-Za-z0-9.\-!]/","",trim($reg_classes));
			if (strlen($reg_classes) > 2000) $reg_classes = mb_substr($reg_classes, 0, 2000); // C'est juste pour éviter une tentative d'overflow...
			...
			if(!$fp) {
				...
				while (!feof($fp)) {
					...
					if(trim($ligne)!="") {
						...
						if (strlen($tabligne[2]) > 2000) $tabligne[2] = mb_substr($tabligne[2], 0, 2000);
