...
if (!isset($_POST["action"])) {
	...
	if ($_POST['action'] == "save_data") {
		...
		while ($lig=mysqli_fetch_object($res_temp)) {
			...
			$test = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT login FROM eleves WHERE elenoet = '" . $reg_id_int . "'");
			...
			if(!$fp) {
				...
				for ($i=0;$i<$k;$i++) {
					if(isset($data_tab[$i]["id_int"])) {
						...
						col2='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $data_tab[$i]["options"]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))."';";
