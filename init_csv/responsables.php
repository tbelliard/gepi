...
if (!isset($_POST["action"])) {
	...
	if ($_POST['action'] == "save_data") {
		...
		while ($lig=mysql_fetch_object($res_temp)) {
			...
			if (strlen($reg_nom) > 50) $reg_nom = mb_substr($reg_nom, 0, 50);
			...
			if (strlen($reg_prenom) > 50) $reg_prenom = mb_substr($reg_prenom, 0, 50);
			...
			if (strlen($reg_adresse1) > 50) $reg_adresse1 = mb_substr($reg_adresse1, 0, 50);
			...
			if (strlen($reg_adresse2) > 50) $reg_adresse2 = mb_substr($reg_adresse2, 0, 50);

			$reg_code_postal = preg_replace("/[^0-9]/","",trim($reg_code_postal));
			if (strlen($reg_code_postal) > 6) $reg_code_postal = mb_substr($reg_code_postal, 0, 6);
			...
			if (strlen($reg_commune) > 50) $reg_commune = mb_substr($reg_commune, 0, 50);
			...
			if(!$fp) {
				...
				while (!feof($fp)) {
					...
					if(trim($ligne)!="") {
						...
						if (strlen($tabligne[1]) > 50) $tabligne[1] = mb_substr($tabligne[1], 0, 50);
						...
						if (strlen($tabligne[2]) > 50) $tabligne[2] = mb_substr($tabligne[2], 0, 50);
						...
						if (strlen($tabligne[4]) > 50) $tabligne[4] = mb_substr($tabligne[4], 0, 50);
						...
						if (strlen($tabligne[5]) > 50) $tabligne[5] = mb_substr($tabligne[5], 0, 50);

						$tabligne[6] = preg_replace("/[^0-9]/","",trim($tabligne[6]));
						if (strlen($tabligne[6]) > 6) $tabligne[6] = mb_substr($tabligne[6], 0, 6);
						...
						if (strlen($tabligne[7]) > 50) $tabligne[7] = mb_substr($tabligne[7], 0, 50);
