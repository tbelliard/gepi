			...
			if(!$fp) {
				...
				while (!feof($fp)) {
					...
					if(trim($ligne)!="") {
						...
						$tabligne[1] = preg_replace("/Æ/","AE",preg_replace("/æ/","ae",preg_replace("/¼/","OE",preg_replace("/½/","oe",preg_replace("/[^A-Za-z .\-àâäéèêëîïôöùûüçÀÄÂÉÈÊËÎÏÔÖÙÛÜÇ]/","",trim(my_strtoupper($tabligne[1])))))));
