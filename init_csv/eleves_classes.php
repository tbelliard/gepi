		...
		while ($lig=mysqli_fetch_object($res_temp)) {
			...
			if(($reg_id_int!='')&&($reg_classe!='')){
				...
				if (mysqli_num_rows($test) == 1) {
					...
					$sql="SELECT id FROM classes WHERE classe = '" . ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $reg_classe) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")) . "'";
					//echo "$sql<br />";
					$test = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
					...
					if (mysqli_num_rows($test) == 0) {
						...
						$sql="INSERT INTO classes SET " .
							"classe = '" . ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $reg_classe) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")) . "', " .
							"nom_complet = '" . ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $reg_classe) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")) . "', " .
							"format_nom = 'np', " .
							"display_rang = 'n', " .
							"display_address = 'n', " .
							"display_coef = 'y'";
						//echo "$sql<br />";
						$insert1 = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
						// On récupère l'ID de la classe nouvelle créée, pour enregistrer les périodes
						$classe_id = ((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);
						...
						for ($p=1;$p<4;$p++) {
							...
							//echo "$sql<br />";
							$insert2 = mysqli_query($GLOBALS["___mysqli_ston"], $sql);
						}
						$num_periods = 3;
						...
						$num_periods = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(num_periode) FROM periodes WHERE id_classe = '" . $classe_id . "'"), 0);
					}
			...
			if(!$fp) {
				...
				for ($i=0;$i<$k;$i++) {
					if(isset($data_tab[$i]["id_int"])) {
						...
						col2='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $data_tab[$i]["classe"]) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))."';";
						$insert=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
						if(!$insert) {
