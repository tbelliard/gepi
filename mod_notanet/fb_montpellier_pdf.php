<?php
	...
	/* $Id: fb_montpellier_pdf.php 8060 2011-08-30 21:58:08Z jjacquard $ */
...
*/
	...
	*/
	...
	$larg_col_note_glob=30;

	// Socles B2i et A2
	//$h_bloc_socles=3*$fs_txt_mm*$sc_interligne;
	$pdf->SetFont('Arial','',$fs_txt);
	$h_bloc_socles=3*$pdf->FontSize*$sc_interligne+1;

	// Bloc nom/adresse etab
	$larg_col_etab=55;
	//$larg_col_val_etab=210-2*$marge-$larg_col_etab;
	$larg_col_val_etab=210-($marge+$larg_col_disc+$larg_col_note+$larg_col_etab+$larg_col_note_glob+$marge);

	// Tableau bloc disciplines
	$larg_col_classe_3eme_college=210-$x_col_note_mc-$larg_col_note_glob-$marge;
	...
	for($i=0;$i<count($id_classe);$i++) {
		...
		if(mysql_num_rows($res1)>0) {
			...
			while($lig1=mysql_fetch_object($res1)) {
				...
				$pdf->Cell($larg_col_prenom,$pdf->FontSize*$sc_interligne, "Prénom(s) : ".$lig1->prenom,$bord_debug,1,'');

				if($ele_lieu_naissance=='y') {
					$lieu_naiss=get_commune($lig1->lieu_naissance, 2);
				}
				else {
					$lieu_naiss="";
				}
				...
				if($lig1->sexe=='F') {
					$pdf->Cell($larg_col_nom,$pdf->FontSize*$sc_interligne, "Née le : ".formate_date($lig1->naissance)." à ".$lieu_naiss,$bord_debug,1,'');
				}
				else {
				...
				$pdf->SetXY($marge+$larg_col_disc+$larg_col_note,$y_etab);
				$pdf->Cell($larg_col_etab+$larg_col_val_etab,$y-$y_etab, "",'LRBT',1,'');
				...
				//$pdf->SetXY($marge,80);
				//$x=$pdf->GetX();
				$pdf->SetXY($marge+$larg_col_disc,$pdf->GetY());
				...
				$y=$y_disc;
				$h_cell=15;
				$largeur_dispo=$larg_col_note_glob;
				$taille_max_police=$fs_txt;
				$taille_min_police=ceil($fs_txt/3);
				$texte='Note Globale affectée du coefficient';
				cell_ajustee(traite_accents_utf8($texte),$x,$y,$largeur_dispo,$h_cell,$taille_max_police,$taille_min_police,'LRBT','C','C');
				//$pdf->drawTextBox('Note Globale affectée du coefficient', $larg_col_note_glob, 15, 'C', 'M', 1);
				...
				for($j=$indice_premiere_matiere;$j<=$indice_max_matieres;$j++) {
					...
					if($tabmatieres[$j][0]!='') {
						...
						if(($tabmatieres[$j][-4]!='non dispensee dans l etablissement')&&($tabmatieres[$j]['socle']!='y')) {
							...
							if($temoin_notnonca==0) {
								...
								if($tabmatieres[$j][-1]!='PTSUP') {
									$pdf->SetXY($x_col_note_glob,$y);
									if($num_fb_col==1) {
										$pdf->Cell($larg_col_note_glob,$h_par_matiere, $t_col1,'LRBT',2,'C');
									}
									else {
										$pdf->Cell($larg_col_note_glob,$h_par_matiere, $t_col2,'LRBT',2,'C');
									}
								}
								else {
									$pdf->SetFont('Arial','',$fs_txt);
				...
				//for($j=$indice_premiere_matiere;$j<=$indice_max_matieres;$j++) {
					...
					//$sql="SELECT note,note_notanet FROM notanet WHERE login='$lig1->login' AND id_classe='$id_classe[$i]' AND notanet_mat='".$tabmatieres[114][0]."';";
					...
					/*
					$sql="SELECT note,note_notanet FROM notanet WHERE login='$lig1->login' AND id_classe='$id_classe[$i]' AND notanet_mat='".$tabmatieres[115][0]."';";
					$res_a2=mysql_query($sql);
					if(mysql_num_rows($res_a2)==0) {
						$note_a2="";
					}
					else {
						$lig_a2=mysql_fetch_object($res_a2);
						$note_a2=$lig_a2->note;
					}
					*/
					...
					//if(($tabmatieres[$j][0]!='')&&($tabmatieres[$j]['socle']=='y')) {
						...
						$pdf->SetXY($marge,$y+$h_txt_socles);
						$pdf->Cell(210-2*$marge, $h_txt_socles, "Niveau A2 de compétence en langue acquis :",'',0,'L');
						$pdf->SetXY($x+$esp,$y+$h_txt_socles+1);
						//$pdf->SetXY(170,170);
						//$pdf->SetXY($x+15,$y);
						$x0=$pdf->GetX();
						$pdf->Cell(4,4, "",'LRBT',0,'C');
						...
						$texte=" Oui";
						$pdf->Cell($pdf->GetStringWidth($texte), $h_txt_socles, $texte,'',0,'C');
						$pdf->SetXY($pdf->GetX()+$esp,$y+$h_txt_socles+1);
						$x0=$pdf->GetX();
						$pdf->Cell(4,4, "",'LRBT',0,'C');
						$x1=$pdf->GetX();
				...
				$pdf->SetXY($marge,$y_cadre_bas);
				$pdf->SetFont('Arial','B',$fs_txt);
				$texte="RÉSULTATS DE L'EXAMEN";
				$pdf->Cell(210-$larg_col_note_glob-2*$marge, $pdf->FontSize*$sc_interligne, $texte,'',1,'C');

				// Tableau des résultats du brevet
				// Ligne 1
				$pdf->SetXY($marge+15,$pdf->GetY());
				$pdf->SetFont('Arial','',$fs_txt);
				$texte="Français";
				$larg_col_bb1=$pdf->GetStringWidth($texte)*$sc_interligne; // On prend le même rapport pour mettre une marge en largeur que pour l'interligne
				$pdf->Cell($larg_col_bb1, $pdf->FontSize*$sc_interligne, $texte,'LRBT',0,'L');

				$texte="        /40";
				$larg_col_bb3=$pdf->GetStringWidth($texte)*$sc_interligne; // On prend le même rapport pour mettre une marge en largeur que pour l'interligne

				$pdf->Cell(210-$larg_col_note_glob-2*$marge-$larg_col_bb1-$larg_col_bb3-15*2, $pdf->FontSize*$sc_interligne, "",'LRBT',0,'L');
				$x_av_col_note=$pdf->GetX();
				$pdf->Cell($larg_col_bb3, $pdf->FontSize*$sc_interligne, $texte,'LRBT',1,'R');
				...
				// Sous le tableau
				$texte="Total des épreuves écrites :";
				$larg_av_col_note=$pdf->GetStringWidth($texte);

				$pdf->SetXY($x_av_col_note-$larg_av_col_note,$pdf->GetY());
				$pdf->Cell($larg_av_col_note, $pdf->FontSize*$sc_interligne, $texte,'',0,'L');
				$texte="        /120";
				$pdf->Cell($larg_col_bb3, $pdf->FontSize*$sc_interligne, $texte,'',1,'R');
...
*/
				...
				// Décision
				$x=210-$marge-$larg_col_note_glob;
				$pdf->SetXY($x,$y_cadre_bas);
				$pdf->Cell($larg_col_note_glob,$h_cadre_bas, "",'LRBT',0,'C');

				$pdf->SetXY($x,$y_cadre_bas);
				$pdf->SetFont('Arial','B',$fs_txt);
				$pdf->Cell($larg_col_note_glob, $pdf->FontSize*$sc_interligne, "DÉCISION",'',1,'C');
				$pdf->SetXY($x,$pdf->GetY());
				$pdf->SetFont('Arial','',$fs_txt);
				$texte="Le candidat";
				$font_size=adjust_size_font($texte,$larg_col_note,$fs_txt,0.3);
				$pdf->SetFontSize($font_size);
				$pdf->Cell($larg_col_note_glob, $pdf->FontSize*$sc_interligne, "Le candidat",'',1,'C');
				$pdf->SetXY($x,$pdf->GetY());
				$pdf->Cell($larg_col_note_glob, $pdf->FontSize*$sc_interligne, "est déclaré",'',1,'C');

				$pdf->SetXY($x+5,$pdf->GetY());
				$pdf->Cell(4,4, "",'LRBT',0,'C');
				$pdf->SetFont('Arial','',$fs_txt);
				$pdf->Cell($larg_col_note_glob-9, $pdf->FontSize*$sc_interligne, " ADMIS",'',1,'C');

				$pdf->SetXY($x+5,$pdf->GetY());
				$pdf->Cell(4,4, "",'LRBT',0,'C');
				$pdf->Cell($larg_col_note_glob-9, $pdf->FontSize*$sc_interligne, " ÉLIMINÉ",'',1,'C');

				$pdf->SetXY($x,$pdf->GetY());
				$texte="Le président du jury";
				$font_size=adjust_size_font($texte,$larg_col_note_glob,$fs_txt,0.3);
				$pdf->SetFont('Arial','I',$fs_txt);
				$pdf->SetFontSize($font_size);
				$pdf->Cell($larg_col_note_glob, $pdf->FontSize*$sc_interligne, $texte,'',1,'C');
