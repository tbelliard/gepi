...
if ($_POST['step'] == "6") {
    ...
    foreach($classes as $classe_courante) {
        ...
        if (array_key_exists($classe_courante->getNom(), $donnees_equipes)) {
          ...
          $students_query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT login FROM j_eleves_classes WHERE 
          ...
          foreach($donnees_equipes[$classe_courante->getNom()] as $login_prof) {
            ...
            if ($prof) {
              ...
              foreach($matieres as $matiere) {
                  ...
                  $rec_groupe = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO groupes SET
                      name = '".$matiere->getNomComplet()."',
                      description = '".$matiere->getNomComplet()."'");
                  $id_groupe = ((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);
                  
                  $rec_mat = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_groupes_matieres SET
                      id_matiere = '".$matiere->getMatiere()."',
                      id_groupe = '".$id_groupe."'");
                  
                  $rec_prof = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_groupes_professeurs SET
                      login = '".$prof->getLogin()."',
                      id_groupe = '".$id_groupe."'");
                  ...
                  foreach ($students as $student) {
                    ...
                    foreach ($classe_courante->getPeriodeNotes() as $periode) {
                      $rec = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_eleves_groupes SET
                                  login = '".$student."',
                                  id_groupe = '".$id_groupe."',
                  ...
                  $rec = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO j_groupes_classes SET
                                  id_groupe = '".$id_groupe."',
