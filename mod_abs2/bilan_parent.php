...
<?php
	if($temoin_abs_ou_retard_ce_jour==0) {
		echo "<p style='margin-top:1em;' class='bold'>Aucune absence saisie le ".french_strftime("%A %d/%m/%Y", $dt_debut->getTimestamp()).".</p>";
	}
?>
        ...
        while ($date_actuelle >= $dt_debut_toutes) {
          ...
          if ($affichage) {
			...
			if($tmp_tab[1]!=$mois_precedent) {
				echo '<tr class="white_hover"><td colspan="'.($creneau_col->count() + 1).'" style="text-align:center; background-color: gray;">'.ucfirst(french_strftime("%B %Y", mktime(13,59,0,$tmp_tab[1],$tmp_tab[0],$tmp_tab[2]))).'</td></tr>';
    ...
    ?>
            <tr class='white_hover' style="background-color :<?php echo $background_couleur;?>">
              <td style="text-align:center;"><?php
              	...
              	$tmp_date_actuelle=french_strftime("%A", mktime(13,59,0,$tmp_tab[1],$tmp_tab[0],$tmp_tab[2]))." ".$tmp_date_actuelle;
