...
if (isset($_POST['is_posted']) and $_POST['is_posted'] == "yes") {
	...
	$quels_eleves = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT e.login FROM eleves e, j_eleves_classes c WHERE (c.id_classe='$id_classe' AND e.login = c.login AND c.periode='$periode_num')");
	...
	while($j < $lignes) {
		...
		if($num_eleve!=-1){
			...
			if (!(preg_match ("/^[0-9]{1,}$/", $nb_absences))) {
				...
				$test_eleve_nb_absences_query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM absences WHERE (login='$reg_eleve_login' AND periode='$periode_num')");
				$test_nb = mysqli_num_rows($test_eleve_nb_absences_query);
				if ($test_nb != "0") {
					$register = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE absences SET nb_absences='$nb_absences', non_justifie='$nb_nj', nb_retards='$nb_retard', appreciation='$ap' WHERE (login='$reg_eleve_login' AND periode='$periode_num')");
				} else {
					$register = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO absences SET login='$reg_eleve_login', periode='$periode_num',nb_absences='$nb_absences',non_justifie='$nb_nj', nb_retards='$nb_retard',appreciation='$ap'");
				}
			if (!$register) {
