	...
	*/
	...
	if($temoin_commentaires_types=="oui"){
		...
		if(mysql_num_rows($resultat_commentaire)>0){
			...
			echo "<input type='text' name='textarea_courant' id='textarea_courant' value='no_anti_inject_current_eleve_login_ap' />\n";
			...
			while($ligne_commentaire=mysql_fetch_object($resultat_commentaire)) {
				...
				if((ereg("<",$ligne_commentaire->commentaire))&&(ereg(">",$ligne_commentaire->commentaire))){
					...
					*/
					...
					//echo "<div style='border: 1px solid black; margin: 1px; padding: 1px;' onClick=\"textarea_courant=document.getElementById('textarea_courant').value;alert('textarea_courant='+textarea_courant);document.getElementById(textarea_courant).value=document.getElementById(textarea_courant).value+document.getElementById('commentaire_type_'+$cpt).value; document.getElementById('commentaire_type').style.display='none'; document.getElementById(textarea_courant).focus();\">\n";
