								...
								if((!isset($responsables[$i]["paie_frais_scolaires"]))||($responsables[$i]["paie_frais_scolaires"]!=1)) {
									...
									if(mysqli_num_rows($res_test)>0) {
										...
										if(mysqli_num_rows($res_test)>0) {
											...
											$info_action_texte="L'élève <a href='".$gepiPath."/eleves/modify_eleve.php?eleve_login=$lig_ele->login'>".$current_ele." a un nombre de responsables avec niveau de responsabilité 1 trop élevé.<br />Un choix a dû être effectué lors de la mise à jour pour en désigner un <a href='".$gepiPath."/responsables/modify_resp.php?pers_id=".$responsables[$i]["personne_id"]."'>responsable légal 1</a>, un autre <a href='".$gepiPath."/responsables/modify_resp.php?pers_id=".$lig_resp_legal->pers_id."'>responsable légal 2</a> et <a href='".$gepiPath."/responsables/modify_resp.php?pers_id=".$lig_resp_legal2->pers_id."'>le ou les autres</a> ont dû être déclarés non responsables légaux.<br />Vous devriez peut-être y jeter un oeil pour corriger éventuellement.";
