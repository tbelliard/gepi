...
// --------------------------------------------------------------------------------
  ...
  function PclTarHandleAddFile($p_tar, $p_filename, $p_mode, &$p_header, $p_add_dir, $p_remove_dir)
  {
    ...
    if ($p_remove_dir != "")
    {
      ...
      if (mb_substr($p_filename, 0, strlen($p_remove_dir)) == $p_remove_dir)
      {
        $v_stored_filename = mb_substr($p_filename, strlen($p_remove_dir));
      ...
      if ((!$v_extract_all) && (is_array($p_file_list)))
      {
        ...
        for ($i=0; $i<sizeof($p_file_list); $i++)
        {
          ...
          if (mb_substr($p_file_list[$i], -1) == "/")
          {
            ...
            if ((strlen($v_header[filename]) > strlen($p_file_list[$i])) && (mb_substr($v_header[filename], 0, strlen($p_file_list[$i])) == $p_file_list[$i]))
            {
...
//christian
  ...
  function PclTarHandleExtractByIndexList($p_tarname, $p_index_string, &$p_list_detail, $p_path, $p_remove_path, $p_tar_mode)
  {
    ...
    // ----- Check the path
    if (($p_path == "") || ((mb_substr($p_path, 0, 1) != "/") && (mb_substr($p_path, 0, 3) != "../") && (mb_substr($p_path, 0, 2) != "./")))
      $p_path = "./".$p_path;
...
//        if ($p_file_list[$i] == $v_header[filename])
        if (($v_len = strcmp($p_file_list[$i], $v_header[filename])) <= 0)
        {
          ...
          else
          {
            ...
            if (mb_substr($v_header[filename], strlen($p_file_list[$i]), 1) == "/")
            {
  ...
  function PclTarHandleUpdate($p_tarname, $p_file_list, &$p_list_detail, $p_tar_mode, $p_add_dir, $p_remove_dir)
  {
    ...
    if ($p_remove_dir != "")
    {
      ...
      if (mb_substr($p_file_list[$i], 0, strlen($p_remove_dir)) == $p_remove_dir)
      {
        $v_stored_list[$i] = mb_substr($p_file_list[$i], strlen($p_remove_dir));
