...
if (isset($is_posted) and ($is_posted == "yes")) {
	...
	while ($i < $nb) {
		...
		if (($reg_sexe != "M") and ($reg_sexe != "F")) {
			$reg_sexe = "M";
		}
		if ($reg_naissance == '') {
			$reg_naissance = "19000101";
		}
		...
		if (($reg_regime != "0") and ($reg_regime != "4") and ($reg_regime != "2") and ($reg_regime != "3")) {
			$regime = "d/p";
		}
		if ($reg_doublant == "O") {
			$doublant = 'R';
	...
	if ($max_lignes_pb == 0) {
		...
		$texte = "";
		if ($affiche != 'partiel') {
			$texte = "--> Pour n'afficher que les lignes ou des problèmes ont été détectés, cliquez sur le bouton \"Affichage partiel\" :";
		} else {
			$texte = "--> Pour afficher toutes les lignes, cliquez sur le bouton \"Afficher tout\" :";
		}
		echo "<p>$texte</p>";
		echo "<form enctype='multipart/form-data' action='step3.php' method=post>";
		echo "<input type='hidden' name='is_posted' value='no' />";
		echo "<input type='hidden' name='affiche' value='" . ($affiche == 'partiel' ? 'partiel' : 'tout') . "' />";
		echo "<center><input type='submit' value='Affichage " . ($affiche == 'partiel' ? 'partiel' : 'tout') . "' /></center>";
		echo "</form>";
	}
