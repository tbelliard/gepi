...
abstract class BaseCahierTexteCompteRenduFichierJoint extends BaseObject implements Persistent
{
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = CahierTexteCompteRenduFichierJointQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::ID_CT)) {
            $modifiedColumns[':p' . $index++]  = 'ID_CT';
        }
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::TITRE)) {
            $modifiedColumns[':p' . $index++]  = 'TITRE';
        }
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::TAILLE)) {
            $modifiedColumns[':p' . $index++]  = 'TAILLE';
        }
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::EMPLACEMENT)) {
            $modifiedColumns[':p' . $index++]  = 'EMPLACEMENT';
        }
        if ($this->isColumnModified(CahierTexteCompteRenduFichierJointPeer::VISIBLE_ELEVE_PARENT)) {
            $modifiedColumns[':p' . $index++]  = 'VISIBLE_ELEVE_PARENT';
        }

        $sql = sprintf(
            'INSERT INTO ct_documents (%s) VALUES (%s)',
