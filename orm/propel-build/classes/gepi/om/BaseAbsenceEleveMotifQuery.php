 ...
 * @method     AbsenceEleveMotifQuery orderByOrdre($order = Criteria::ASC) Order by the ordre column
 *
 ...
 * @method     AbsenceEleveMotifQuery groupByOrdre() Group by the ordre column
 *
 ...
 * @method     AbsenceEleveMotif findOneByOrdre(int $ordre) Return the first AbsenceEleveMotif filtered by the ordre column
 *
 ...
 * @method     array findByOrdre(int $ordre) Return AbsenceEleveMotif objects filtered by the ordre column
 *
...
abstract class BaseAbsenceEleveMotifQuery extends ModelCriteria
{
	...
	{
		if (is_array($id)) {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::ID, $id, Criteria::IN);
		} else {
	...
	{
		if (is_array($nom)) {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::NOM, $nom, Criteria::IN);
		} elseif(preg_match('/[\%\*]/', $nom)) {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::NOM, str_replace('*', '%', $nom), Criteria::LIKE);
		} else {
	...
	public function filterByCommentaire($commentaire = null, $comparison = Criteria::EQUAL)
	{
		if (is_array($commentaire)) {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::COMMENTAIRE, $commentaire, Criteria::IN);
		} elseif(preg_match('/[\%\*]/', $commentaire)) {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::COMMENTAIRE, str_replace('*', '%', $commentaire), Criteria::LIKE);
		} else {
	...
	public function filterByOrdre($ordre = null, $comparison = Criteria::EQUAL)
	{
		if (is_array($ordre)) {
			if (array_values($ordre) === $ordre) {
				return $this->addUsingAlias(AbsenceEleveMotifPeer::ORDRE, $ordre, Criteria::IN);
			} else {
				if (isset($ordre['min'])) {
					$this->addUsingAlias(AbsenceEleveMotifPeer::ORDRE, $ordre['min'], Criteria::GREATER_EQUAL);
				}
				if (isset($ordre['max'])) {
					$this->addUsingAlias(AbsenceEleveMotifPeer::ORDRE, $ordre['max'], Criteria::LESS_EQUAL);
				}
				return $this;	
			}
		} else {
			return $this->addUsingAlias(AbsenceEleveMotifPeer::ORDRE, $ordre, $comparison);
		}
	}
