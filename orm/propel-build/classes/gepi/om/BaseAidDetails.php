...
abstract class BaseAidDetails extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|JAidUtilisateursProfessionnels[] Collection to store aggregation of JAidUtilisateursProfessionnels objects.
     */
    protected $collJAidUtilisateursProfessionnelss;
    protected $collJAidUtilisateursProfessionnelssPartial;
    ...
    /**
     * @var        PropelObjectCollection|Eleve[] Collection to store aggregation of Eleve objects.
     */
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->elevesScheduledForDeletion !== null) {
                if (!$this->elevesScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    $pk = $this->getPrimaryKey();
                    foreach ($this->elevesScheduledForDeletion->getPrimaryKeys(false) as $remotePk) {
                        $pks[] = array($pk, $remotePk);
                    }
    ...
    public function clearJAidUtilisateursProfessionnelss()
    {
        ...
        $this->collJAidUtilisateursProfessionnelssPartial = null;
    }

    /**
     * reset is the collJAidUtilisateursProfessionnelss collection loaded partially
     *
     * @return void
     */
    public function resetPartialJAidUtilisateursProfessionnelss($v = true)
    {
        $this->collJAidUtilisateursProfessionnelssPartial = $v;
    }
     ...
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|JAidUtilisateursProfessionnels[] List of JAidUtilisateursProfessionnels objects
     * @throws PropelException
     */
    public function getJAidUtilisateursProfessionnelss($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collJAidUtilisateursProfessionnelssPartial && !$this->isNew();
        if (null === $this->collJAidUtilisateursProfessionnelss || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collJAidUtilisateursProfessionnelss) {
                ...
                $collJAidUtilisateursProfessionnelss = JAidUtilisateursProfessionnelsQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collJAidUtilisateursProfessionnelssPartial && count($collJAidUtilisateursProfessionnelss)) {
                      $this->initJAidUtilisateursProfessionnelss(false);

                      foreach($collJAidUtilisateursProfessionnelss as $obj) {
                        if (false == $this->collJAidUtilisateursProfessionnelss->contains($obj)) {
                          $this->collJAidUtilisateursProfessionnelss->append($obj);
                        }
                      }

                      $this->collJAidUtilisateursProfessionnelssPartial = true;
                    }

                    return $collJAidUtilisateursProfessionnelss;
                }

                if($partial && $this->collJAidUtilisateursProfessionnelss) {
                    foreach($this->collJAidUtilisateursProfessionnelss as $obj) {
                        if($obj->isNew()) {
                            $collJAidUtilisateursProfessionnelss[] = $obj;
                        }
                    }
                }

                $this->collJAidUtilisateursProfessionnelss = $collJAidUtilisateursProfessionnelss;
                $this->collJAidUtilisateursProfessionnelssPartial = false;
            }
        }
    ...
    public function setJAidUtilisateursProfessionnelss(PropelCollection $jAidUtilisateursProfessionnelss, PropelPDO $con = null)
    {
        ...
        foreach ($this->jAidUtilisateursProfessionnelssScheduledForDeletion as $jAidUtilisateursProfessionnelsRemoved) {
            $jAidUtilisateursProfessionnelsRemoved->setAidDetails(null);
        }
    ...
    public function countJAidUtilisateursProfessionnelss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collJAidUtilisateursProfessionnelss || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collJAidUtilisateursProfessionnelss) {
                ...
                if($partial && !$criteria) {
                    return count($this->getJAidUtilisateursProfessionnelss());
                }
    ...
    public function addJAidUtilisateursProfessionnels(JAidUtilisateursProfessionnels $l)
    {
        ...
        if (!in_array($l, $this->collJAidUtilisateursProfessionnelss->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    /**
     * @param	JAidUtilisateursProfessionnels $jAidUtilisateursProfessionnels The jAidUtilisateursProfessionnels object to remove.
     */
    public function removeJAidUtilisateursProfessionnels($jAidUtilisateursProfessionnels)
    {
        if ($this->getJAidUtilisateursProfessionnelss()->contains($jAidUtilisateursProfessionnels)) {
            $this->collJAidUtilisateursProfessionnelss->remove($this->collJAidUtilisateursProfessionnelss->search($jAidUtilisateursProfessionnels));
            if (null === $this->jAidUtilisateursProfessionnelssScheduledForDeletion) {
                $this->jAidUtilisateursProfessionnelssScheduledForDeletion = clone $this->collJAidUtilisateursProfessionnelss;
                $this->jAidUtilisateursProfessionnelssScheduledForDeletion->clear();
            }
            $this->jAidUtilisateursProfessionnelssScheduledForDeletion[]= $jAidUtilisateursProfessionnels;
            $jAidUtilisateursProfessionnels->setAidDetails(null);
        }
    }
    ...
    public function setJAidElevess(PropelCollection $jAidElevess, PropelPDO $con = null)
    {
        ...
        foreach ($this->jAidElevessScheduledForDeletion as $jAidElevesRemoved) {
            $jAidElevesRemoved->setAidDetails(null);
        }
    ...
    public function setAbsenceEleveSaisies(PropelCollection $absenceEleveSaisies, PropelPDO $con = null)
    {
        ...
        foreach ($this->absenceEleveSaisiesScheduledForDeletion as $absenceEleveSaisieRemoved) {
            $absenceEleveSaisieRemoved->setAidDetails(null);
        }
    ...
    public function removeAbsenceEleveSaisie($absenceEleveSaisie)
    {
        if ($this->getAbsenceEleveSaisies()->contains($absenceEleveSaisie)) {
            ...
            $absenceEleveSaisie->setAidDetails(null);
        }
    }
    ...
    public function setEdtEmplacementCourss(PropelCollection $edtEmplacementCourss, PropelPDO $con = null)
    {
        ...
        foreach ($this->edtEmplacementCourssScheduledForDeletion as $edtEmplacementCoursRemoved) {
            $edtEmplacementCoursRemoved->setAidDetails(null);
        }
    ...
    public function removeEdtEmplacementCours($edtEmplacementCours)
    {
        if ($this->getEdtEmplacementCourss()->contains($edtEmplacementCours)) {
            ...
            $edtEmplacementCours->setAidDetails(null);
        }
    }
     ...
     * @param PropelPDO $con Optional connection object
     *
     * @return PropelObjectCollection|Eleve[] List of Eleve objects
     */
    ...
    {
        $this->clearEleves();
        $currentEleves = $this->getEleves();

        $this->elevesScheduledForDeletion = $currentEleves->diff($eleves);

        foreach ($eleves as $eleve) {
            if (!$currentEleves->contains($eleve)) {
    ...
    /**
     * Remove a Eleve object to this object
     * through the j_aid_eleves cross reference table.
     *
     * @param Eleve $eleve The JAidEleves object to relate
     * @return void
    ...
    public function removeEleve(Eleve $eleve)
    {
        if ($this->getEleves()->contains($eleve)) {
            $this->collEleves->remove($this->collEleves->search($eleve));
            if (null === $this->elevesScheduledForDeletion) {
                $this->elevesScheduledForDeletion = clone $this->collEleves;
                $this->elevesScheduledForDeletion->clear();
            }
            $this->elevesScheduledForDeletion[]= $eleve;
        }
    }
