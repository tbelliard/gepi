...
abstract class BaseCategorieMatiere extends BaseObject implements Persistent
{
    ...
    /**
     * reset is the collMatieres collection loaded partially
     *
     * @return void
     */
    public function resetPartialMatieres($v = true)
    {
        $this->collMatieresPartial = $v;
    }
    ...
    {
        $partial = $this->collMatieresPartial && !$this->isNew();
        if (null === $this->collMatieres || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collMatieres) {
                ...
                $collMatieres = MatiereQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collMatieresPartial && count($collMatieres)) {
                      $this->initMatieres(false);

                      foreach($collMatieres as $obj) {
                        if (false == $this->collMatieres->contains($obj)) {
                          $this->collMatieres->append($obj);
                        }
                      }

                      $this->collMatieresPartial = true;
                    }

                    return $collMatieres;
                }

                if($partial && $this->collMatieres) {
                    foreach($this->collMatieres as $obj) {
                        if($obj->isNew()) {
                            $collMatieres[] = $obj;
                        }
                    }
                ...
                $this->collMatieresPartial = false;
            }
        }
    ...
    public function setMatieres(PropelCollection $matieres, PropelPDO $con = null)
    {
        ...
        foreach ($this->matieresScheduledForDeletion as $matiereRemoved) {
            $matiereRemoved->setCategorieMatiere(null);
        }
    ...
    public function addMatiere(Matiere $l)
    {
        ...
        if (!in_array($l, $this->collMatieres->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddMatiere($l);
        }
    ...
    /**
     * @param	Matiere $matiere The matiere object to remove.
     */
    public function removeMatiere($matiere)
    {
        if ($this->getMatieres()->contains($matiere)) {
            ...
            $matiere->setCategorieMatiere(null);
        }
    }
    ...
    public function setJCategoriesMatieresClassess(PropelCollection $jCategoriesMatieresClassess, PropelPDO $con = null)
    {
        ...
        foreach ($this->jCategoriesMatieresClassessScheduledForDeletion as $jCategoriesMatieresClassesRemoved) {
            $jCategoriesMatieresClassesRemoved->setCategorieMatiere(null);
        }
    ...
    public function removeJCategoriesMatieresClasses($jCategoriesMatieresClasses)
    {
        if ($this->getJCategoriesMatieresClassess()->contains($jCategoriesMatieresClasses)) {
            ...
            $jCategoriesMatieresClasses->setCategorieMatiere(null);
        }
    }
