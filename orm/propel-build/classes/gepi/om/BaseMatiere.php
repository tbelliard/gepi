...
abstract class BaseMatiere extends BaseObject implements Persistent
{
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->professeursScheduledForDeletion !== null) {
                if (!$this->professeursScheduledForDeletion->isEmpty()) {
                    $pks = array();
                    $pk = $this->getPrimaryKey();
                    foreach ($this->professeursScheduledForDeletion->getPrimaryKeys(false) as $remotePk) {
    ...
    public function setJGroupesMatieress(PropelCollection $jGroupesMatieress, PropelPDO $con = null)
    {
        ...
        foreach ($this->jGroupesMatieressScheduledForDeletion as $jGroupesMatieresRemoved) {
            $jGroupesMatieresRemoved->setMatiere(null);
        }
    ...
    public function removeJGroupesMatieres($jGroupesMatieres)
    {
        if ($this->getJGroupesMatieress()->contains($jGroupesMatieres)) {
            ...
            $jGroupesMatieres->setMatiere(null);
        }
    }
    ...
    public function setJProfesseursMatieress(PropelCollection $jProfesseursMatieress, PropelPDO $con = null)
    {
        ...
        foreach ($this->jProfesseursMatieressScheduledForDeletion as $jProfesseursMatieresRemoved) {
            $jProfesseursMatieresRemoved->setMatiere(null);
        }
    ...
    public function removeJProfesseursMatieres($jProfesseursMatieres)
    {
        if ($this->getJProfesseursMatieress()->contains($jProfesseursMatieres)) {
            ...
            $jProfesseursMatieres->setMatiere(null);
        }
    }
    ...
    public function clearProfesseurs()
    {
        ...
        $this->collProfesseursPartial = null;
    }
    ...
    {
        $this->clearProfesseurs();
        $currentProfesseurs = $this->getProfesseurs();

        $this->professeursScheduledForDeletion = $currentProfesseurs->diff($professeurs);

        foreach ($professeurs as $professeur) {
            if (!$currentProfesseurs->contains($professeur)) {
                $this->doAddProfesseur($professeur);
            }
        }
     ...
     * @return void
     */
    public function removeProfesseur(UtilisateurProfessionnel $utilisateurProfessionnel)
    {
        if ($this->getProfesseurs()->contains($utilisateurProfessionnel)) {
            $this->collProfesseurs->remove($this->collProfesseurs->search($utilisateurProfessionnel));
            if (null === $this->professeursScheduledForDeletion) {
                $this->professeursScheduledForDeletion = clone $this->collProfesseurs;
                $this->professeursScheduledForDeletion->clear();
            }
            $this->professeursScheduledForDeletion[]= $utilisateurProfessionnel;
        }
    }
