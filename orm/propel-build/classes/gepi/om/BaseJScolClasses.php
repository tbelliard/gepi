...
/**
 * Base class that represents a row from the 'j_scol_classes' table.
 *
 * Table permettant le jointure entre un utilisateur scolarite et une classe.
 *
 * @package    propel.generator.gepi.om
 */
abstract class BaseJScolClasses extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'JScolClassesPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        JScolClassesPeer
	 */
	...
	public function setLogin($v)
	{
		...
		if ($this->login !== $v) {
			$this->login = $v;
			$this->modifiedColumns[] = JScolClassesPeer::LOGIN;
		}
	...
	public function setIdClasse($v)
	{
		...
		if ($this->id_classe !== $v) {
			$this->id_classe = $v;
			$this->modifiedColumns[] = JScolClassesPeer::ID_CLASSE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = JScolClassesPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating JScolClasses object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = JScolClassesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(JScolClassesPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
		}
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				JScolClassesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += JScolClassesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = JScolClassesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = JScolClassesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['JScolClasses'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['JScolClasses'][serialize($this->getPrimaryKey())] = true;
		$keys = JScolClassesPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(JScolClassesPeer::LOGIN)) $criteria->add(JScolClassesPeer::LOGIN, $this->login);
		if ($this->isColumnModified(JScolClassesPeer::ID_CLASSE)) $criteria->add(JScolClassesPeer::ID_CLASSE, $this->id_classe);
	...
	{
		$criteria = new Criteria(JScolClassesPeer::DATABASE_NAME);
		$criteria->add(JScolClassesPeer::LOGIN, $this->login);
		$criteria->add(JScolClassesPeer::ID_CLASSE, $this->id_classe);
	...
	{
		return (null === $this->getLogin()) && (null === $this->getIdClasse());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of JScolClasses (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     JScolClasses Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     JScolClassesPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new JScolClassesPeer();
		}
		return self::$peer;
	}
	...
	{
		return (string) $this->exportTo(JScolClassesPeer::DEFAULT_STRING_FORMAT);
	}
