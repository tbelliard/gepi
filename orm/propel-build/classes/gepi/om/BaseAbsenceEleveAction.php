...
abstract class BaseAbsenceEleveAction extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
  const PEER = 'AbsenceEleveActionPeer';
	...
	public function setSortableRank($v)
	{
		...
		if ($this->sortable_rank !== $v) {
			...
			$this->modifiedColumns[] = AbsenceEleveActionPeer::SORTABLE_RANK;
		}
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			AbsenceEleveActionPeer::shiftRank(-1, $this->getSortableRank() + 1, null, $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				// sortable behavior
				if (!$this->isColumnModified(AbsenceEleveActionPeer::RANK_COL)) {
					$this->setSortableRank(AbsenceEleveActionQuery::create()->getMaxRank($con) + 1);
				}
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AbsenceEleveActionPeer::ID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.AbsenceEleveActionPeer::ID.')');
					}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AbsenceEleveActionPeer::SORTABLE_RANK)) $criteria->add(AbsenceEleveActionPeer::SORTABLE_RANK, $this->sortable_rank);
	 ...
	 * If this AbsenceEleveAction is new, it will return
		...
		if(null === $this->collAbsenceEleveTraitements || null !== $criteria) {
			if ($this->isNew() && null === $this->collAbsenceEleveTraitements) {
				...
				$collAbsenceEleveTraitements = AbsenceEleveTraitementQuery::create(null, $criteria)
					->filterByAbsenceEleveAction($this)
					->find($con);
				if (null !== $criteria) {
	...
	{
		return $this->getSortableRank() == AbsenceEleveActionQuery::create()->getMaxRank($con);
	}
	...
	{
		return AbsenceEleveActionQuery::create()->findOneByRank($this->getSortableRank() + 1, $con);
	}
	 ...
	 * @param     PropelPDO  $con      optional connection
	 *
	 * @return    AbsenceEleveAction the current object
	 *
	 * @throws    PropelException
	 */
	public function insertAtRank($rank, PropelPDO $con = null)
	{
		$maxRank = AbsenceEleveActionQuery::create()->getMaxRank($con);
		...
		if ($rank != $maxRank + 1) {
			...
			$this->sortableQueries []= array(
				'callable'  => array('AbsenceEleveActionPeer', 'shiftRank'),
	...
	public function moveToRank($newRank, PropelPDO $con = null)
	{
		...
		if ($newRank < 1 || $newRank > AbsenceEleveActionQuery::create()->getMaxRank($con)) {
		...
		try {
			...
			AbsenceEleveActionPeer::shiftRank($delta, min($oldRank, $newRank), max($oldRank, $newRank), $con);
	 ...
	 *
	 * @param     AbsenceEleveAction $object
	 * @param     PropelPDO $con optional connection
	 *
	 * @return    AbsenceEleveAction the current object
	 *
	...
	public function moveToBottom(PropelPDO $con = null)
	{
		...
		$con->beginTransaction();
		try {
			$bottom = AbsenceEleveActionQuery::create()->getMaxRank($con);
