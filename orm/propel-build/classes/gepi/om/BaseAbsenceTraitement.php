...
abstract class BaseAbsenceTraitement extends BaseObject  implements Persistent {
	 ...
	 * related JTraitementSaisieAbsences from storage. If this AbsenceTraitement is new, it will return
		...
		if ($this->collJTraitementSaisieAbsences === null) {
			if ($this->isNew()) {
			   $this->collJTraitementSaisieAbsences = array();
				...
				$criteria->add(JTraitementSaisieAbsencePeer::A_TRAITEMENT_ID, $this->id);
	 ...
	 */
	public function getJTraitementSaisieAbsencesJoinAbsenceSaisie($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AbsenceTraitementPeer::DATABASE_NAME);
		}
		...
		if ($this->collJTraitementSaisieAbsences === null) {
			if ($this->isNew()) {
				...
				$this->collJTraitementSaisieAbsences = JTraitementSaisieAbsencePeer::doSelectJoinAbsenceSaisie($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
