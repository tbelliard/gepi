...
abstract class BaseJEleveCpe extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the e_login field.
	...
	/**
	 * The value for the cpe_login field.
	 * Note: this column has a database default value of: ''
	 * @var        string
	 */
	protected $cpe_login;
	...
	/**
	 * Get the [cpe_login] column value.
	 * cle etrangere, login du CPE (utilisateur professionnel)
	 * @return     string
	 */
	public function getCpeLogin()
	{
		return $this->cpe_login;
	}

	/**
	 * Set the value of [e_login] column.
	 * cle etrangere, login de l'eleve
	 * @param      string $v new value
	 * @return     JEleveCpe The current object (for fluent API support)
	 */
	public function setELogin($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
	...
	/**
	 * Set the value of [cpe_login] column.
	 * cle etrangere, login du CPE (utilisateur professionnel)
	 * @param      string $v new value
	 * @return     JEleveCpe The current object (for fluent API support)
	 */
	public function setCpeLogin($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->e_login = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->cpe_login = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = JEleveCpePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating JEleveCpe object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aEleve !== null && $this->e_login !== $this->aEleve->getLogin()) {
			$this->aEleve = null;
		}
		if ($this->aUtilisateurProfessionnel !== null && $this->cpe_login !== $this->aUtilisateurProfessionnel->getLogin()) {
			$this->aUtilisateurProfessionnel = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = JEleveCpePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = JEleveCpeQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(JEleveCpePeer::E_LOGIN)) {
			$modifiedColumns[':p' . $index++]  = 'E_LOGIN';
		}
		if ($this->isColumnModified(JEleveCpePeer::CPE_LOGIN)) {
			$modifiedColumns[':p' . $index++]  = 'CPE_LOGIN';
		}

		$sql = sprintf(
			'INSERT INTO j_eleves_cpe (%s) VALUES (%s)',
		...
		try {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					case 'E_LOGIN':
						$stmt->bindValue($identifier, $this->e_login, PDO::PARAM_STR);
						break;
					case 'CPE_LOGIN':
						$stmt->bindValue($identifier, $this->cpe_login, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = JEleveCpePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = JEleveCpePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				return $this->getELogin();
				break;
			case 1:
				return $this->getCpeLogin();
				break;
			default:
				return null;
				break;
	...
	{
		if (isset($alreadyDumpedObjects['JEleveCpe'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['JEleveCpe'][serialize($this->getPrimaryKey())] = true;
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setELogin($value);
				break;
			case 1:
				$this->setCpeLogin($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setELogin($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setCpeLogin($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(JEleveCpePeer::E_LOGIN)) $criteria->add(JEleveCpePeer::E_LOGIN, $this->e_login);
		if ($this->isColumnModified(JEleveCpePeer::CPE_LOGIN)) $criteria->add(JEleveCpePeer::CPE_LOGIN, $this->cpe_login);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of JEleveCpe (or compatible) type.
	...
	{
		$copyObj->setELogin($this->getELogin());
		$copyObj->setCpeLogin($this->getCpeLogin());
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     JEleveCpe Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     JEleveCpePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new JEleveCpePeer();
		}
		return self::$peer;
	}
	...
	{
		return (string) $this->exportTo(JEleveCpePeer::DEFAULT_STRING_FORMAT);
	}
