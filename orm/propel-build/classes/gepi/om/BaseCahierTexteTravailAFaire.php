...
abstract class BaseCahierTexteTravailAFaire extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CahierTexteTravailAFaireFichierJoint[] Collection to store aggregation of CahierTexteTravailAFaireFichierJoint objects.
     */
    protected $collCahierTexteTravailAFaireFichierJoints;
    protected $collCahierTexteTravailAFaireFichierJointsPartial;
     ...
     */
    protected $cahierTexteTravailAFaireFichierJointsScheduledForDeletion = null;
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::ID_CT)) {
            $modifiedColumns[':p' . $index++]  = 'ID_CT';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::DATE_CT)) {
            $modifiedColumns[':p' . $index++]  = 'DATE_CT';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::CONTENU)) {
            $modifiedColumns[':p' . $index++]  = 'CONTENU';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::VISE)) {
            $modifiedColumns[':p' . $index++]  = 'VISE';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::ID_GROUPE)) {
            $modifiedColumns[':p' . $index++]  = 'ID_GROUPE';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::ID_LOGIN)) {
            $modifiedColumns[':p' . $index++]  = 'ID_LOGIN';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::ID_SEQUENCE)) {
            $modifiedColumns[':p' . $index++]  = 'ID_SEQUENCE';
        }
        if ($this->isColumnModified(CahierTexteTravailAFairePeer::DATE_VISIBILITE_ELEVE)) {
            $modifiedColumns[':p' . $index++]  = 'DATE_VISIBILITE_ELEVE';
        }

        $sql = sprintf(
            'INSERT INTO ct_devoirs_entry (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'ID_SEQUENCE':
                        ...
                        break;
                    case 'DATE_VISIBILITE_ELEVE':
                        $stmt->bindValue($identifier, $this->date_visibilite_eleve, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function clearCahierTexteTravailAFaireFichierJoints()
    {
        ...
        $this->collCahierTexteTravailAFaireFichierJointsPartial = null;
    }

    /**
     * reset is the collCahierTexteTravailAFaireFichierJoints collection loaded partially
     *
     * @return void
     */
    public function resetPartialCahierTexteTravailAFaireFichierJoints($v = true)
    {
        $this->collCahierTexteTravailAFaireFichierJointsPartial = $v;
    }
     ...
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CahierTexteTravailAFaireFichierJoint[] List of CahierTexteTravailAFaireFichierJoint objects
     * @throws PropelException
     */
    public function getCahierTexteTravailAFaireFichierJoints($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCahierTexteTravailAFaireFichierJointsPartial && !$this->isNew();
        if (null === $this->collCahierTexteTravailAFaireFichierJoints || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCahierTexteTravailAFaireFichierJoints) {
                ...
                $collCahierTexteTravailAFaireFichierJoints = CahierTexteTravailAFaireFichierJointQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCahierTexteTravailAFaireFichierJointsPartial && count($collCahierTexteTravailAFaireFichierJoints)) {
                      $this->initCahierTexteTravailAFaireFichierJoints(false);

                      foreach($collCahierTexteTravailAFaireFichierJoints as $obj) {
                        if (false == $this->collCahierTexteTravailAFaireFichierJoints->contains($obj)) {
                          $this->collCahierTexteTravailAFaireFichierJoints->append($obj);
                        }
                      }

                      $this->collCahierTexteTravailAFaireFichierJointsPartial = true;
                    }

                    return $collCahierTexteTravailAFaireFichierJoints;
                }

                if($partial && $this->collCahierTexteTravailAFaireFichierJoints) {
                    foreach($this->collCahierTexteTravailAFaireFichierJoints as $obj) {
                        if($obj->isNew()) {
                            $collCahierTexteTravailAFaireFichierJoints[] = $obj;
                        }
                    }
                }

                $this->collCahierTexteTravailAFaireFichierJoints = $collCahierTexteTravailAFaireFichierJoints;
                $this->collCahierTexteTravailAFaireFichierJointsPartial = false;
            }
        }
    ...
    /**
     * Sets a collection of CahierTexteTravailAFaireFichierJoint objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $cahierTexteTravailAFaireFichierJoints A Propel collection.
     * @param PropelPDO $con Optional connection object
     */
    public function setCahierTexteTravailAFaireFichierJoints(PropelCollection $cahierTexteTravailAFaireFichierJoints, PropelPDO $con = null)
    {
        $this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion = $this->getCahierTexteTravailAFaireFichierJoints(new Criteria(), $con)->diff($cahierTexteTravailAFaireFichierJoints);

        foreach ($this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion as $cahierTexteTravailAFaireFichierJointRemoved) {
            $cahierTexteTravailAFaireFichierJointRemoved->setCahierTexteTravailAFaire(null);
        }

        $this->collCahierTexteTravailAFaireFichierJoints = null;
        foreach ($cahierTexteTravailAFaireFichierJoints as $cahierTexteTravailAFaireFichierJoint) {
            $this->addCahierTexteTravailAFaireFichierJoint($cahierTexteTravailAFaireFichierJoint);
        }

        $this->collCahierTexteTravailAFaireFichierJoints = $cahierTexteTravailAFaireFichierJoints;
        $this->collCahierTexteTravailAFaireFichierJointsPartial = false;
    }
    ...
    public function countCahierTexteTravailAFaireFichierJoints(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collCahierTexteTravailAFaireFichierJoints || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCahierTexteTravailAFaireFichierJoints) {
                ...
                if($partial && !$criteria) {
                    return count($this->getCahierTexteTravailAFaireFichierJoints());
                }
    ...
    public function addCahierTexteTravailAFaireFichierJoint(CahierTexteTravailAFaireFichierJoint $l)
    {
        ...
        if (!in_array($l, $this->collCahierTexteTravailAFaireFichierJoints->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCahierTexteTravailAFaireFichierJoint($l);
        }
    ...
    /**
     * @param	CahierTexteTravailAFaireFichierJoint $cahierTexteTravailAFaireFichierJoint The cahierTexteTravailAFaireFichierJoint object to remove.
     */
    public function removeCahierTexteTravailAFaireFichierJoint($cahierTexteTravailAFaireFichierJoint)
    {
        if ($this->getCahierTexteTravailAFaireFichierJoints()->contains($cahierTexteTravailAFaireFichierJoint)) {
            $this->collCahierTexteTravailAFaireFichierJoints->remove($this->collCahierTexteTravailAFaireFichierJoints->search($cahierTexteTravailAFaireFichierJoint));
            if (null === $this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion) {
                $this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion = clone $this->collCahierTexteTravailAFaireFichierJoints;
                $this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion->clear();
            }
            $this->cahierTexteTravailAFaireFichierJointsScheduledForDeletion[]= $cahierTexteTravailAFaireFichierJoint;
            $cahierTexteTravailAFaireFichierJoint->setCahierTexteTravailAFaire(null);
        }
    }
