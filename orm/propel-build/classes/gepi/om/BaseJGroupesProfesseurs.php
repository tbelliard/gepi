...
abstract class BaseJGroupesProfesseurs extends BaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = JGroupesProfesseursPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(JGroupesProfesseursPeer::ID_GROUPE)) {
			$modifiedColumns[':p' . $index++]  = 'ID_GROUPE';
		}
		if ($this->isColumnModified(JGroupesProfesseursPeer::LOGIN)) {
			$modifiedColumns[':p' . $index++]  = 'LOGIN';
		}

		$sql = sprintf(
			'INSERT INTO j_groupes_professeurs (%s) VALUES (%s)',
	...
	{
		if (isset($alreadyDumpedObjects['JGroupesProfesseurs'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['JGroupesProfesseurs'][serialize($this->getPrimaryKey())] = true;
	...
	{
		return (string) $this->exportTo(JGroupesProfesseursPeer::DEFAULT_STRING_FORMAT);
	}
