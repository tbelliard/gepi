...
abstract class BaseAbsenceEleveTypeStatutAutoriseQuery extends ModelCriteria
{
	...
	{
		if (is_array($id)) {
			return $this->addUsingAlias(AbsenceEleveTypeStatutAutorisePeer::ID, $id, Criteria::IN);
		} else {
	 ...
	 * @return    AbsenceEleveTypeStatutAutoriseQuery The current query, for fluid interface
	 */
	public function filterByIdAType($idAType = null, $comparison = Criteria::EQUAL)
	{
		if (is_array($idAType)) {
			if (array_values($idAType) === $idAType) {
				return $this->addUsingAlias(AbsenceEleveTypeStatutAutorisePeer::ID_A_TYPE, $idAType, Criteria::IN);
			} else {
	...
	{
		if (is_array($statut)) {
			return $this->addUsingAlias(AbsenceEleveTypeStatutAutorisePeer::STATUT, $statut, Criteria::IN);
		} elseif(preg_match('/[\%\*]/', $statut)) {
			return $this->addUsingAlias(AbsenceEleveTypeStatutAutorisePeer::STATUT, str_replace('*', '%', $statut), Criteria::LIKE);
		} else {
