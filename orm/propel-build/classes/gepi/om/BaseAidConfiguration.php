...
abstract class BaseAidConfiguration extends BaseObject implements Persistent
{
    ...
    protected $collAidDetailssPartial;
    ...
    /**
     * reset is the collAidDetailss collection loaded partially
     *
     * @return void
     */
    public function resetPartialAidDetailss($v = true)
    {
        $this->collAidDetailssPartial = $v;
    }
    ...
    {
        $partial = $this->collAidDetailssPartial && !$this->isNew();
        if (null === $this->collAidDetailss || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collAidDetailss) {
                ...
                $collAidDetailss = AidDetailsQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collAidDetailssPartial && count($collAidDetailss)) {
                      $this->initAidDetailss(false);

                      foreach($collAidDetailss as $obj) {
                        if (false == $this->collAidDetailss->contains($obj)) {
                          $this->collAidDetailss->append($obj);
                        }
                      }

                      $this->collAidDetailssPartial = true;
                    }

                    return $collAidDetailss;
                }

                if($partial && $this->collAidDetailss) {
                    foreach($this->collAidDetailss as $obj) {
                        if($obj->isNew()) {
                            $collAidDetailss[] = $obj;
                        }
                    }
                ...
                $this->collAidDetailssPartial = false;
            }
        }
    ...
    public function setAidDetailss(PropelCollection $aidDetailss, PropelPDO $con = null)
    {
        ...
        foreach ($this->aidDetailssScheduledForDeletion as $aidDetailsRemoved) {
            $aidDetailsRemoved->setAidConfiguration(null);
        }
    ...
    public function countAidDetailss(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collAidDetailss || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collAidDetailss) {
                ...
                if($partial && !$criteria) {
                    return count($this->getAidDetailss());
                }
    ...
    public function addAidDetails(AidDetails $l)
    {
        ...
        if (!in_array($l, $this->collAidDetailss->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    /**
     * @param	AidDetails $aidDetails The aidDetails object to remove.
     */
    public function removeAidDetails($aidDetails)
    {
        if ($this->getAidDetailss()->contains($aidDetails)) {
            ...
            $aidDetails->setAidConfiguration(null);
        }
    }
