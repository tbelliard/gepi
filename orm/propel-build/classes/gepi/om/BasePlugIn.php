...
abstract class BasePlugIn extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|PlugInAutorisation[] Collection to store aggregation of PlugInAutorisation objects.
     */
    protected $collPlugInAutorisations;
    protected $collPlugInAutorisationsPartial;

    /**
     * @var        PropelObjectCollection|PlugInMiseEnOeuvreMenu[] Collection to store aggregation of PlugInMiseEnOeuvreMenu objects.
     */
    protected $collPlugInMiseEnOeuvreMenus;
    protected $collPlugInMiseEnOeuvreMenusPartial;
    ...
    public function clearPlugInAutorisations()
    {
        ...
        $this->collPlugInAutorisationsPartial = null;
    }

    /**
     * reset is the collPlugInAutorisations collection loaded partially
     *
     * @return void
     */
    public function resetPartialPlugInAutorisations($v = true)
    {
        $this->collPlugInAutorisationsPartial = $v;
    }
     ...
     * @return PropelObjectCollection|PlugInAutorisation[] List of PlugInAutorisation objects
     * @throws PropelException
     */
    public function getPlugInAutorisations($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collPlugInAutorisationsPartial && !$this->isNew();
        if (null === $this->collPlugInAutorisations || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collPlugInAutorisations) {
                ...
                $collPlugInAutorisations = PlugInAutorisationQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collPlugInAutorisationsPartial && count($collPlugInAutorisations)) {
                      $this->initPlugInAutorisations(false);

                      foreach($collPlugInAutorisations as $obj) {
                        if (false == $this->collPlugInAutorisations->contains($obj)) {
                          $this->collPlugInAutorisations->append($obj);
                        }
                      }

                      $this->collPlugInAutorisationsPartial = true;
                    }

                    return $collPlugInAutorisations;
                }

                if($partial && $this->collPlugInAutorisations) {
                    foreach($this->collPlugInAutorisations as $obj) {
                        if($obj->isNew()) {
                            $collPlugInAutorisations[] = $obj;
                        }
                    }
                }

                $this->collPlugInAutorisations = $collPlugInAutorisations;
                $this->collPlugInAutorisationsPartial = false;
            }
        }
    ...
    public function setPlugInAutorisations(PropelCollection $plugInAutorisations, PropelPDO $con = null)
    {
        ...
        foreach ($this->plugInAutorisationsScheduledForDeletion as $plugInAutorisationRemoved) {
            $plugInAutorisationRemoved->setPlugIn(null);
        }
    ...
    public function countPlugInAutorisations(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collPlugInAutorisations || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collPlugInAutorisations) {
                ...
                if($partial && !$criteria) {
                    return count($this->getPlugInAutorisations());
                }
    ...
    public function addPlugInAutorisation(PlugInAutorisation $l)
    {
        ...
        if (!in_array($l, $this->collPlugInAutorisations->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    /**
     * @param	PlugInAutorisation $plugInAutorisation The plugInAutorisation object to remove.
     */
    public function removePlugInAutorisation($plugInAutorisation)
    {
        if ($this->getPlugInAutorisations()->contains($plugInAutorisation)) {
            $this->collPlugInAutorisations->remove($this->collPlugInAutorisations->search($plugInAutorisation));
            if (null === $this->plugInAutorisationsScheduledForDeletion) {
                $this->plugInAutorisationsScheduledForDeletion = clone $this->collPlugInAutorisations;
                $this->plugInAutorisationsScheduledForDeletion->clear();
            }
            $this->plugInAutorisationsScheduledForDeletion[]= $plugInAutorisation;
            $plugInAutorisation->setPlugIn(null);
        }
    }
    ...
    public function clearPlugInMiseEnOeuvreMenus()
    {
        ...
        $this->collPlugInMiseEnOeuvreMenusPartial = null;
    }

    /**
     * reset is the collPlugInMiseEnOeuvreMenus collection loaded partially
     *
     * @return void
     */
    public function resetPartialPlugInMiseEnOeuvreMenus($v = true)
    {
        $this->collPlugInMiseEnOeuvreMenusPartial = $v;
    }
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|PlugInMiseEnOeuvreMenu[] List of PlugInMiseEnOeuvreMenu objects
     * @throws PropelException
     */
    public function getPlugInMiseEnOeuvreMenus($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collPlugInMiseEnOeuvreMenusPartial && !$this->isNew();
        if (null === $this->collPlugInMiseEnOeuvreMenus || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collPlugInMiseEnOeuvreMenus) {
                ...
                $collPlugInMiseEnOeuvreMenus = PlugInMiseEnOeuvreMenuQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collPlugInMiseEnOeuvreMenusPartial && count($collPlugInMiseEnOeuvreMenus)) {
                      $this->initPlugInMiseEnOeuvreMenus(false);

                      foreach($collPlugInMiseEnOeuvreMenus as $obj) {
                        if (false == $this->collPlugInMiseEnOeuvreMenus->contains($obj)) {
                          $this->collPlugInMiseEnOeuvreMenus->append($obj);
                        }
                      }

                      $this->collPlugInMiseEnOeuvreMenusPartial = true;
                    }

                    return $collPlugInMiseEnOeuvreMenus;
                }

                if($partial && $this->collPlugInMiseEnOeuvreMenus) {
                    foreach($this->collPlugInMiseEnOeuvreMenus as $obj) {
                        if($obj->isNew()) {
                            $collPlugInMiseEnOeuvreMenus[] = $obj;
                        }
                    }
                }

                $this->collPlugInMiseEnOeuvreMenus = $collPlugInMiseEnOeuvreMenus;
                $this->collPlugInMiseEnOeuvreMenusPartial = false;
            }
        }
    ...
    public function setPlugInMiseEnOeuvreMenus(PropelCollection $plugInMiseEnOeuvreMenus, PropelPDO $con = null)
    {
        ...
        foreach ($this->plugInMiseEnOeuvreMenusScheduledForDeletion as $plugInMiseEnOeuvreMenuRemoved) {
            $plugInMiseEnOeuvreMenuRemoved->setPlugIn(null);
        }
    ...
    public function countPlugInMiseEnOeuvreMenus(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collPlugInMiseEnOeuvreMenus || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collPlugInMiseEnOeuvreMenus) {
                ...
                if($partial && !$criteria) {
                    return count($this->getPlugInMiseEnOeuvreMenus());
                }
    ...
    public function addPlugInMiseEnOeuvreMenu(PlugInMiseEnOeuvreMenu $l)
    {
        ...
        if (!in_array($l, $this->collPlugInMiseEnOeuvreMenus->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    /**
     * @param	PlugInMiseEnOeuvreMenu $plugInMiseEnOeuvreMenu The plugInMiseEnOeuvreMenu object to remove.
     */
    public function removePlugInMiseEnOeuvreMenu($plugInMiseEnOeuvreMenu)
    {
        if ($this->getPlugInMiseEnOeuvreMenus()->contains($plugInMiseEnOeuvreMenu)) {
            $this->collPlugInMiseEnOeuvreMenus->remove($this->collPlugInMiseEnOeuvreMenus->search($plugInMiseEnOeuvreMenu));
            if (null === $this->plugInMiseEnOeuvreMenusScheduledForDeletion) {
                $this->plugInMiseEnOeuvreMenusScheduledForDeletion = clone $this->collPlugInMiseEnOeuvreMenus;
                $this->plugInMiseEnOeuvreMenusScheduledForDeletion->clear();
            }
            $this->plugInMiseEnOeuvreMenusScheduledForDeletion[]= $plugInMiseEnOeuvreMenu;
            $plugInMiseEnOeuvreMenu->setPlugIn(null);
        }
    }
