...
if ($_SESSION['statut'] == 'eleve') {
	...
	if ($selected_eleve) {
		if (my_strtolower($selected_eleve->login) != my_strtolower($_SESSION['login'])) {
			tentative_intrusion(2, "Tentative d'un élève d'accéder au cahier de textes d'un autre élève.");
		}
	}
	...
	if (mysqli_num_rows($get_eleves) == 1) {
		...
		while ($test = mysqli_fetch_object($get_eleves)) {
			if (my_strtolower($test->login) == my_strtolower($selected_eleve->login)) {
				$ok = true;
			}
		}
		if (!$ok) {
