...
if (!isset($_POST["action"])) {
	...
	if ($_POST['action'] == "save_data") {
		...
		while ($go) {
			...
			if (strlen($reg_options) > 2000) $reg_options = mb_substr($reg_options, 0, 2000); // Juste pour éviter une tentative d'overflow...
				...
				//=========================
					...
					while (!feof($fp)) {
						...
						if(trim($ligne)!="") {
							...
							if (strlen($tabligne[1]) > 2000) $tabligne[1] = mb_substr($tabligne[1], 0, 2000);
