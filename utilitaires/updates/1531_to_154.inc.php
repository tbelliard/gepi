...
$result .= "<h3 class='titreMaJ'>Mise à jour vers la version 1.5.4" . " :</h3>";
...
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'cas_attribut_prenom'");
...
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'cas_attribut_nom'");
$res_test=mysqli_num_rows($req_test);
if ($res_test==0){
...
$req_test=mysqli_query($GLOBALS["mysqli"], "SELECT value FROM setting WHERE name = 'cas_attribut_email'");
$res_test=mysqli_num_rows($req_test);
if ($res_test==0){
...
$test_date_decompte=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM a_types LIKE 'id_lieu';"));
if ($test_date_decompte>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE a_types ADD id_lieu INTEGER(11) COMMENT 'cle etrangere du lieu ou se trouve l\'eleve' AFTER commentaire,
...
$test_date_decompte=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM a_saisies LIKE 'id_lieu';"));
if ($test_date_decompte>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE a_saisies ADD id_lieu INTEGER(11) COMMENT 'cle etrangere du lieu ou se trouve l\'eleve' AFTER modifie_par_utilisateur_id,
...
$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE cc_notes_eval CHANGE statut statut VARCHAR( 4 ) NOT NULL;");
if ($query) {
        $result .= msj_ok();
} else {
        $result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM eb_epreuves LIKE 'note_sur';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "alter table eb_epreuves add note_sur int(11) unsigned not null default '20';");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'nombre_jours';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` ADD `nombre_jours` VARCHAR( 50 ) NOT NULL ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'qualification_faits';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` ADD `qualification_faits` text NOT NULL ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'num_courrier';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` ADD `num_courrier` VARCHAR( 50 ) NOT NULL ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'type_exclusion';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` ADD `type_exclusion` VARCHAR( 50 ) NOT NULL ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'fct_delegation';"));
if ($test_note_sur<0) {
	$result .= msj_present(" Le champ n'existe plus.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` DROP `fct_delegation` ;");
	if ($query) {
			$result .= msj_present(" Le champ n'existe plus.");
	} else {
			$result .= msj_ok();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'fct_autorite';"));
if ($test_note_sur<0) {
	$result .= msj_present("> Le champ n'existe plus.");
}
else {
    $query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` DROP `fct_autorite` ;");
	if ($query) {
			$result .= msj_present(" Le champ n'existe plus.");
	} else {
			$result .= msj_ok();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'nom_autorite';"));
if ($test_note_sur<0) {
	$result .= msj_present("> Le champ n'existe plus.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` DROP `nom_autorite` ;");
	if ($query) {
			$result .= msj_present(" Le champ n'existe plus.");
	} else {
			$result .= msj_ok();
...
$test_note_sur=mysqli_num_rows(mysqli_query($GLOBALS["mysqli"], "SHOW COLUMNS FROM s_exclusions LIKE 'id_signataire';"));
if ($test_note_sur>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["mysqli"], "ALTER TABLE `s_exclusions` ADD `id_signataire` INT NOT NULL ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$res_csrf=mysqli_query($GLOBALS["mysqli"], $sql);
