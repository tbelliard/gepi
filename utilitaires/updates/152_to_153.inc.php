...
$result .= "<br /><br /><b>Mise à jour du module mod_abs2 :</b><br />";
...
// ============= Insertion d'un champ pour CDT2 - gestion des aids

$result .= "<p>-> Insertion d'un champ pour CDT2 - gestion des aids</p>";

$sql = "SELECT id_aid FROM ct_entry LIMIT 1";
$req_rank = mysql_query($sql);
if (!$req_rank){
    $sql_request = "ALTER TABLE ct_entry ADD id_aid INT(11) DEFAULT '0' ";
    $req_add_rank = mysql_query($sql_request);
    if ($req_add_rank) {
        $result .= "<p style=\"color:green;\">Ajout du champ id_aid dans la table <strong>ct_entry</strong> : ok.</p>";
    }
    else {
        $result .= "<p style=\"color:red;\">Ajout du champ id_aid à la table <strong>ct_entry</strong> : Erreur.</p>".mysql_error();
    }
}
else {
    $result .= "<p style=\"color:blue;\">Ajout du champ id_aid à la table <strong>ct_entry</strong> : déjà réalisé.</p>";
}
// ============= Insertion d'un champ pour CDT2 - gestion des aids

$sql = "SELECT id_aid FROM ct_private_entry LIMIT 1";
$req_rank = mysql_query($sql);
if (!$req_rank){
    $sql_request = "ALTER TABLE cdt_private_entry ADD id_aid INT(11) DEFAULT '0' ";
    $req_add_rank = mysql_query($sql_request);
    if ($req_add_rank) {
        $result .= "<p style=\"color:green;\">Ajout du champ id_aid dans la table <strong>ct_private_entry</strong> : ok.</p>";
    }
    else {
        $result .= "<p style=\"color:red;\">Ajout du champ id_aid à la table <strong>ct_private_entry</strong> : Erreur.</p>";
    }
}
else {
    $result .= "<p style=\"color:blue;\">Ajout du champ id_aid à la table <strong>ct_private_entry</strong> : déjà réalisé.</p>";
}

// ============= Insertion d'un champ pour CDT2 - gestion des aids

$sql = "SELECT id_aid FROM ct_devoirs_entry LIMIT 1";
$req_rank = mysql_query($sql);
if (!$req_rank){
    $sql_request = "ALTER TABLE ct_devoirs_entry ADD id_aid INT(11) DEFAULT '0' ";
    $req_add_rank = mysql_query($sql_request);
    if ($req_add_rank) {
        $result .= "<p style=\"color:green;\">Ajout du champ id_aid dans la table <strong>ct_devoirs_entry</strong> : ok.</p>";
    }
    else {
        $result .= "<p style=\"color:red;\">Ajout du champ id_aid à la table <strong>ct_devoirs_entry</strong> : Erreur.</p>";
    }
}
else {
    $result .= "<p style=\"color:blue;\">Ajout du champ id_aid à la table <strong>ct_devoirs_entry</strong> : déjà réalisé.</p>";
}
?>
