...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM ct_documents LIKE 'visible';"));
if ($test_champ>0) {
	$result .= "&nbsp;->Ajout d'un champ visible à la table 'ct_documents'<br />";
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE ct_documents DROP visible;");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM ct_documents LIKE 'visible_eleve_parent';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE ct_documents ADD visible_eleve_parent BOOLEAN DEFAULT true COMMENT 'Visibilité élève/parent du document joint' AFTER emplacement;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM ct_devoirs_documents LIKE 'visible';"));
if ($test_champ>0) {
	$result .= "&nbsp;->Suppression du champ 'visible' de la table 'ct_devoirs_documents'<br />";
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE ct_devoirs_documents DROP visible;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM ct_devoirs_documents LIKE 'visible_eleve_parent';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE ct_devoirs_documents ADD visible_eleve_parent BOOLEAN DEFAULT true COMMENT 'Visibilité élève/parent du document joint' AFTER emplacement;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM ct_devoirs_entry LIKE 'date_visibilite_eleve';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE ct_devoirs_entry ADD date_visibilite_eleve TIMESTAMP NOT NULL default now() COMMENT 'Timestamp precisant quand les devoirs sont portes a la conaissance des eleves' AFTER id_sequence;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM utilisateurs LIKE 'salt';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE utilisateurs ADD salt varchar(128) COMMENT 'sel pour le hmac du mot de passe' AFTER password;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
	}
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE utilisateurs MODIFY password varchar(128);");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM messages LIKE 'statuts_destinataires';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ est déjà modifié.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE messages CHANGE destinataires statuts_destinataires VARCHAR( 10 ) NOT NULL DEFAULT '';");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM messages LIKE 'login_destinataire';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE messages ADD login_destinataire VARCHAR( 50 ) NOT NULL default '' AFTER statuts_destinataires, ADD INDEX ( login_destinataire ) ;");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$req_res=0;
$req_test = mysqli_query($GLOBALS["___mysqli_ston"], "SHOW INDEX FROM messages ");
...
if ($req_res == 0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE messages ADD INDEX login_destinataire ( login_destinataire )");
	if ($query) {
		$result .= msj_ok();
	} else {
		$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_saisies LIKE 'version';"));
if ($test_champ>0) {
	$result .= msj_present("Les versions de saisies existent déjà.");
} else {
	 $query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE a_saisies ADD (version INTEGER DEFAULT 0, version_created_at DATETIME, version_created_by VARCHAR(100));");
	if ($query) {
                $result .= msj_ok();
...
$test = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES LIKE 'a_saisies_version'"));
if ($test!=0) {
	$result .= msj_present("La table des versions de saisies existent déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "CREATE TABLE a_saisies_version
(
	...
	if ($query) {
				...
				$query = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO a_saisies_version (id,utilisateur_id,eleve_id,commentaire,debut_abs,fin_abs,id_edt_creneau,id_edt_emplacement_cours,id_groupe,id_classe,id_aid,id_s_incidents,id_lieu,created_at,updated_at,version,version_created_at,version_created_by)
...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies` DROP `modifie_par_utilisateur_id` ;");
if ($query) {
...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies_version` DROP `modifie_par_utilisateur_id` ;");
if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_saisies LIKE 'deleted_at';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies` ADD deleted_at DATETIME AFTER updated_at;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_saisies_version LIKE 'deleted_at';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies_version` ADD deleted_at DATETIME AFTER updated_at;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_saisies LIKE 'deleted_by';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies` ADD deleted_by VARCHAR(100) AFTER id_lieu;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_saisies_version LIKE 'deleted_by';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_saisies_version` ADD deleted_by VARCHAR(100) AFTER id_lieu;");
	if ($query) {
...
$test = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW TABLES LIKE 'a_agregation_decompte'"));
if ($test!=0) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_traitements LIKE 'deleted_at';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_traitements` ADD deleted_at DATETIME AFTER updated_at;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_types LIKE 'created_at';"));
if ($test_champ>0) {
	$result .= msj_present("Les champs existent déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_types` ADD (created_at DATETIME, updated_at DATETIME);");
	if ($query) {
			$result .= msj_present("Les champs ont étés ajoutés.");
			$query = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE a_types SET created_at = NOW(), updated_at = NOW();");
	} else {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_motifs LIKE 'created_at';"));
if ($test_champ>0) {
	$result .= msj_present("Les champs existent déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_motifs` ADD (created_at DATETIME, updated_at DATETIME);");
	if ($query) {
			$result .= msj_present("Les champs ont étés ajoutés.");
			$query = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE a_motifs SET created_at = NOW(), updated_at = NOW();");
	} else {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM a_justifications LIKE 'created_at';"));
if ($test_champ>0) {
	$result .= msj_present("Les champs existent déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `a_justifications` ADD (created_at DATETIME, updated_at DATETIME);");
	if ($query) {
			$result .= msj_present("Les champs ont étés ajoutés.");
			$query = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE a_justifications SET created_at = NOW(), updated_at = NOW();");
	} else {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM avis_conseil_classe LIKE 'id_mention';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE avis_conseil_classe ADD id_mention INT( 11 ) NOT NULL DEFAULT '0' AFTER avis;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM s_natures LIKE 'id_categorie';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "alter table s_natures add id_categorie int(11) not null default '0' after nature;");
	if ($query) {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM mef LIKE 'mef_code';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ est déjà modifié.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE mef CHANGE ext_id mef_code INTEGER COMMENT 'code mef de la formation de l\'eleve';");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM eleves LIKE 'mef_code';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ est déjà modifié.");
}
else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE eleves CHANGE id_mef mef_code INTEGER COMMENT 'code mef de la formation de l\'eleve';");
	if ($query) {
			$result .= msj_ok();
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM gc_affichages LIKE 'nom_requete';"));
if ($test_champ>0) {
	$result .= msj_present("Le champ existe déjà.");
} else {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE gc_affichages ADD nom_requete VARCHAR( 255 ) NOT NULL AFTER projet;");
	if ($query) {
...
$req_test=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT value FROM setting WHERE name = 'sso_cas_table'");
...
if ($test != -1) {
    ...
    $req = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT `id` FROM `plugins` WHERE `nom`='plugin_sso_table'");
...
MEL varchar(255) NOT NULL default ''
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM temp_gep_import2 LIKE 'LIEU_NAISSANCE';"));
	if ($test_champ==0) {
		$result .= "<br />&nbsp;->Ajout d'un champ 'LIEU_NAISSANCE' à la table 'temp_gep_import2'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE temp_gep_import2 ADD LIEU_NAISSANCE VARCHAR( 50 ) NOT NULL AFTER ELEOPT12;");
		if ($query) {
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM temp_gep_import2 LIKE 'MEL';"));
	if ($test_champ==0) {
		$result .= "<br />&nbsp;->Ajout d'un champ 'MEL' à la table 'temp_gep_import2'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE temp_gep_import2 ADD MEL VARCHAR( 255 ) NOT NULL AFTER LIEU_NAISSANCE;");
		if ($query) {
...
if ($test != -1) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tempo_utilisateurs_resp CHANGE password password VARCHAR( 128 ) NOT NULL DEFAULT '';");
	if ($query) {
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM tempo_utilisateurs_resp LIKE 'salt';"));
	if ($test_champ==0) {
		$result .= "<br />&nbsp;->Ajout d'un champ 'salt' à la table 'tempo_utilisateurs_resp'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tempo_utilisateurs_resp ADD salt VARCHAR( 128 ) NOT NULL AFTER password;");
		if ($query) {
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM tempo_utilisateurs_resp LIKE 'email';"));
	if ($test_champ==0) {
		$result .= "<br />&nbsp;->Ajout d'un champ 'email' à la table 'tempo_utilisateurs_resp'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tempo_utilisateurs_resp ADD email VARCHAR( 50 ) NOT NULL AFTER salt;");
		if ($query) {
...
if ($test_tempo_utilisateurs!=-1) {
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM tempo_utilisateurs LIKE 'pers_id';"));
	if ($test_champ!=0) {
		$result .= "<br />&nbsp;->Changement du champ 'pers_id' de la table 'tempo_utilisateurs' en 'identifiant1'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tempo_utilisateurs CHANGE pers_id identifiant1 VARCHAR( 10 ) NOT NULL;");
		if ($query) {
	...
	$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM tempo_utilisateurs LIKE 'identifiant2';"));
	if ($test_champ==0) {
		$result .= "<br />&nbsp;->Ajout d'un champ 'identifiant2' à la table 'tempo_utilisateurs'<br />";
		$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tempo_utilisateurs ADD identifiant2 VARCHAR( 50 ) NOT NULL AFTER identifiant1;");
		if ($query) {
	...
	if ($test != -1) {
		...
		$test = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT 1=1 FROM tempo_utilisateurs_resp;");
		...
		//if ($test != -1) {
			...
			$test2 = mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE `tempo_utilisateurs`;");
