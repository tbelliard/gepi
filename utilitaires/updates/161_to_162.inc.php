...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM eleves LIKE 'tel_pers';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE eleves ADD tel_pers varchar(255) NOT NULL default '';");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM eleves LIKE 'tel_port';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE eleves ADD tel_port varchar(255) NOT NULL default '';");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM eleves LIKE 'tel_prof';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE eleves ADD tel_prof varchar(255) NOT NULL default '';");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM s_sanctions LIKE 'id_nature_sanction';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE s_sanctions ADD id_nature_sanction int(11) NOT NULL AFTER nature;");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
		...
		if ($result_inter == '') {
			...
			for($loop=0;$loop<count($tab_nature_sanction);$loop++) {
				...
				if(mysqli_num_rows($res_ns)==0) {
					...
					if ($result_inter == '') {
						$id_nature_sanction_courante=((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);
			...
			*/
			...
			$res_sts=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
			while($lig_sts=mysqli_fetch_object($res_sts)) {
				$sql="INSERT INTO s_types_sanctions2 SET nature='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $lig_sts->nature) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))."', type='autre';";
				$result_inter = traite_requete($sql);
				if ($result_inter == '') {
					$id_nature_sanction=((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res);

					$sql="update s_sanctions set id_nature_sanction='$id_nature_sanction', nature='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $lig_sts->nature) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))."' where id_sanction in (select id_sanction from s_autres_sanctions where id_nature='".$lig_sts->id_nature."');";
					$result_inter = traite_requete($sql);
					if ($result_inter == '') {
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM s_incidents LIKE 'primo_declarant';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE s_incidents ADD primo_declarant varchar(50);");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM s_travail_mesure LIKE 'materiel';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE s_travail_mesure ADD materiel varchar(150);");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM s_retenues LIKE 'materiel';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE s_retenues ADD materiel varchar(150);");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE etablissements CHANGE cp cp VARCHAR( 10 ) NOT NULL;");
if ($query) {
	$result .= msj_ok("SUCCES !");
} else {
	$result .= msj_erreur("ECHEC !");
...
$test_champ=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SHOW COLUMNS FROM responsables2 LIKE 'acces_sp';"));
if ($test_champ==0) {
	$query = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE responsables2 ADD acces_sp varchar(1) NOT NULL default '';");
	if ($query) {
			$result .= msj_ok("Ok !");
	} else {
			$result .= msj_erreur();
