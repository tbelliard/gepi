...
//this file is a modification of module/core/www/loginuserpassorg.php
...
if (array_key_exists('organization', $_REQUEST)) {
	...
	$organization = '';
	//let's check if there is en extension for preferred organization
	if (isset($state['saml:Extensions'])) {
		foreach ($state['saml:Extensions'] as $extension) {
			//$extension is type of SAML2_XML_Chunk Class
			if ($extension->localName == 'organization' 
			&& $extension->namespaceURI == 'gepi_name_space' ) {
				$organization = $extension->getXML()->nodeValue;
			}
		}
	}
}
