 ...
 */
function reload(frm) {
    const mois = frm.elements['mois'];
    const annee = frm.elements['annee'];
	//Debug du mois et année
	const index1 = mois.options[mois.selectedIndex].value;
    const index2 = annee.options[annee.selectedIndex].value;
    //Envoi du formulaire
	frm.submit();
}
...
function submitDate(jour) {
    let tab = this.checkNum(jour, this.moisc);
	jour = tab[0];
    let mois = tab[1];
    let val;
