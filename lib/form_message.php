...
if((isset($mode))&&($mode=='check')) {
	...
	if($messages_non_lus!="") {
		echo "<a href='$gepiPath/lib/form_message.php?mode=afficher_messages_non_lus' target='_blank'><img src='$gepiPath/images/icons/new_mail.gif' style='width:16px; height:16px' alt='Nouveaux messages' title='Vous avez $messages_non_lus' /></a>";
		...
		$test=mysql_query($sql);
		if(mysql_num_rows($test)>0) {
			echo "<span id='span_messages_recus'><a href='$gepiPath/lib/form_message.php' target='_blank'><img src='$gepiPath/images/icons/no_mail.png' style='width:16px; height:16px' alt='Aucun message' title='Aucun message' /></a></span>";
		}
		else {
...
if((isset($mode))&&($mode=='check2')) {
	...
	if($messages_non_lus!="") {
		...
		echo "<a href='$gepiPath/lib/form_message.php?mode=afficher_messages_non_lus' target='_blank'><img src='$gepiPath/images/icons/temoin_message_non_lu.gif' style='width:$MessagerieLargeurImg px; height:$MessagerieLargeurImgpx' alt='Nouveaux messages' title='Vous avez $messages_non_lus' /></a>";
	}
	else {
		echo "";
	}
	...
	if(is_numeric($id_msg)) {
		...
		if(!isset($_GET['mode_no_js'])) {
			if($retour=="Erreur") {
				...
				echo "<img src='../images/enabled.png' style='width:20px; height:20px' alt='Activer' title='Lu/vu' />";
			}
		}
		else {
			if($retour=="Erreur") {
	...
	if(is_numeric($id_msg)) {
		...
		if(!isset($_GET['mode_no_js'])) {
			if($retour=="Erreur") {
				...
				echo "<img src='../images/disabled.png' style='width:20px; height:20px' alt='Désactiver' title='Lu/vu' />";
			}
		}
		else {
			if($retour=="Erreur") {
...
if((isset($mode))&&($mode=='clore')) {
	...
	if(is_numeric($id_msg)) {
		...
		if(!isset($_GET['mode_no_js'])) {
			...
			elseif($retour==2) {
				echo "<img src='../images/icons/securite.png' style='width:16px; height:16px' alt='Message clos' title='Message clos/traité.' />";
			}
			else {
				echo "<img src='../images/disabled.png' style='width:16px; height:16px' alt='Non lu' title='Non lu/vu' />";
			}
		}
		else {
			if($retour=="Erreur") {
	...
	if((isset($login_dest))&&(isset($sujet))&&(isset($message))) {
		...
		elseif(($login_dest!='')&&($sujet!='')&&($message!='')) {
			...
			if($retour!="") {
				...
				if(isset($_GET['envoi_js'])) {
					echo "<img src='$gepiPath/images/icons/mail_echec.png' style='width:16px; height:16px' alt='Erreur' title='Erreur lors de l envoi du message' />";
					die();
				}
			}
		}
