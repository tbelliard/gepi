...
function min2HHMM(nb_minutes) {
	//alert('nb_minutes='+nb_minutes)
...
function clavier_heure(n,e) {
	...
	if (touche == '40') {
		...
		if(nb_minutes>0){
			nb_minutes--;
			valeur=min2HHMM(nb_minutes);
			document.getElementById(n).value=valeur;
		}
		else {
			document.getElementById(n).value='23:59';
		}
	}
	else{
		// Flèche HAUT
		if (touche == '38') {
			...
			if(nb_minutes<eval(23*60+59)){
				nb_minutes++;
				valeur=min2HHMM(nb_minutes);
				document.getElementById(n).value=valeur;
			}
			else {
				document.getElementById(n).value='00:00';
			}
		}
		else{
			// Flèche PageDOWN
