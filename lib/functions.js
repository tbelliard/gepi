...
function minutes2HHMM(nb_minutes) {
	let h = Math.floor(nb_minutes / 60);
	m = nb_minutes - h * 60;
	if (h < 10) {
		h = '0' + h;
	}
...
function HHMM(heure, op, nb) {
	...
	const exp = new RegExp("^[0-9]{1,2}:[0-9]{0,2}$", "g");
	let tab,
		nb_minutes;
...
function decalage_date(date, sens, nbj) {
	...
	cur_date = date.split('/');
	cur_jour = cur_date[0];
	if (cur_jour.substr(0, 1) === '0') {
		cur_jour = cur_jour.substr(1);
	}
	cur_mois = cur_date[1];
	if (cur_mois.substr(0, 1) === '0') {
		cur_mois = cur_mois.substr(1);
	}
	cur_an = cur_date[2];
	...
	if (sens === '-') {
		...
		date_modifiee = tab[2] + '/' + tab[1] + '/' + tab[0];
...
function clavier_date2(n, e) {
	...
	let date_heure_courante = document.getElementById(n).value;
	let cur_date;
	let cur_heure;
	let tmp_date;
	let tab_date_heure;
...
function confirmlink(theLink, theSqlQuery, themessage) {
	...
	const is_confirmed = confirm(themessage + ' :\n' + theSqlQuery);
	if (is_confirmed) {
...
function ouvre_popup_reference(url) {
	window.open(url,'fen','width=500,height=600,menubar=no,scrollbars=yes')
	fen.focus();
}
...
function emptyFormElements(theForm, theFieldName) {
	...
	const isRegExp = (typeof (theField.value.replace) != 'undefined');
...
function checkFormElementInRange(theForm, theFieldName, min, max) {
	const theField = theForm.elements[theFieldName];
	const val = parseInt(theField.value);
...
let capsError = 'Le verrouillage des majuscules est activé.\n\nPour éviter toute erreur lors de la saisie du mot de passe, vous devriez le désactiver en pressant à nouveau la touche "caps lock" (ou "ver. maj") à gauche sur votre clavier.';
...
function capsDetect(e) {
	...
	const theKey = e.which ? e.which : (e.keyCode ? e.keyCode : (e.charCode ? e.charCode : 0));
	//was the shift key was pressed
	const theShift = e.shiftKey || (e.modifiers && (e.modifiers & 4)); //bitWise AND
...
function change_mode_header(mode, path) {
	...
	const url = path + '/lib/change_mode_header.php';
	...
	const laRequete = new Ajax.Request(url, o_options);
}
...
function clavier_date_plus_moins(id, e) {
	...
	if (document.getElementById(id)) {
		const touche = e.keyCode;

		//if((touche == '61')||(touche == '109')) {
		if ((touche === '40') || (touche === '38')) {
			const ladate = document.getElementById(id).value;

			//var pattern = new RegExp([0-3][0-9]-(0|1)[0-9]-(19|20)[0-9]{2});
			const pattern = new RegExp("[0-3][0-9]/[0-1][0-9]/[1-2][0-9][0-9][0-9]");
			if (ladate.match(pattern)) {
				const tmp_tab = ladate.split('/');
				let j = tmp_tab[0];
				let m = tmp_tab[1];
				let a = tmp_tab[2];
				...
				if (checkdate(m, j, a)) {
					...
					// Touche Flèche Haut
					if (touche === '40') {
						NewDate.setTime(nbmillisec + 86400000)
					}
					// Touche -
					//if (touche == '109') {
					// Touche Flèche Bas
...
function info_form(id_div) {
	...
	if (forms.length === 0) {
		...
		for (let i = 0; i < forms.length; i++) {
			...
			const champs_input = forms[i].getElementsByTagName('input');
			const champs_textarea = forms[i].getElementsByTagName('textarea');
...
function corriger_espaces_ponctuation(chaine) {
	...
	let chaine2 = chaine.replace(re1, function (w) {
		return w.toUpperCase()
	}).replace(re2, ". $1").replace(/(\s)+/g, "$1");
...
function corriger_espaces_et_casse_ponctuation(chaine) {
	let chaine2 = majuscule_apres_point(chaine);
	//document.getElementById('texte2').value=chaine2;
	//alert('plop');
	return corriger_espaces_ponctuation2(chaine2);
}
...
function clavier_note(n, e) {
	...
	*/
	...
	let mode;
	let valeur;
	if ((touche === '40') || (touche === '65') || (touche === '68')) {
		...
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace(';', '.');
		}
	} else if (touche === '49') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('&', '1');
		}
	} else if (touche === '50') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('é', '2');
		}
	} else if (touche === '51') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('"', '3');
		}
	} else if (touche === '52') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace("'", '4');
		}
	} else if (touche === '53') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('(', '5');
		}
	} else if (touche === '55') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('è', '7');
		}
	} else if (touche === '56') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('_', '8');
		}
	} else if (touche === '57') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('ç', '9');
		}
	} else if (touche === '49') {
		if (document.getElementById(n)) {
			document.getElementById(n).value = document.getElementById(n).value.replace('à', '0');
		}
	}
	/*
	else if (touche == '54') {
	...
	*/
		...
		if (document.getElementById('cn_mode_saisie')) {
			let mode = document.getElementById('cn_mode_saisie').value;
			if (mode === 1) {
				...
				if (touche !== '16') {
					...
					if (document.getElementById(id)) {
						...
						if ((valeur.match(/\.[0-9]+$/)) || (valeur.match(/,[0-9]+$/))) {
							id = "n";
							id = id.concat((parseInt(n.substr(1)) + 1).toString(10));
							if (document.getElementById(id)) document.getElementById(id).focus();
						} else if (touche === '54') {
...
function volume_human(volume, unite) {
	//alert(volume);

	if (unite === 'o') {
		if (volume >= 1048576) {
			volume = Math.round(10 * volume / 1048576) / 10;
			return volume + " Mo";
		} else if (volume >= 1024) {
			volume = Math.round(10 * volume / 1024) / 10;
			return volume + " ko";
		} else {
			return volume + " o";
		}
