				...
				foreach($plugin_xml->administration->menu->item as $menu_script)
					...
					if (in_array($t_abr_statuts[$_SESSION['statut']],$t_autorisations))
						...
						if ($autorise)
							{
							$nb_items++;
							$tmp_sous_menu_plugins.="						<li><a href=\"".$gepiPath."/mod_plugins/".$plugin['nom']."/".$menu_script."\"".insert_confirm_abandon().">".$menu_script->attributes()->titre."</a></li>\n";
							$tmp_sous_menu_plugins_solo="						<li><a href=\"".$gepiPath."/mod_plugins/".$plugin['nom']."/".$menu_script."\"".insert_confirm_abandon().">".$plugin['nom']."&nbsp;"."</a></li>\n";
							}
						}
					}
					if ($nb_items>1)
