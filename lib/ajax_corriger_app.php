...
$corriger_app_id_aid=isset($_POST['corriger_app_id_aid']) ? $_POST['corriger_app_id_aid'] : "";
...
//echo "\$corriger_app_login_eleve=$corriger_app_login_eleve<br />";
//echo "\$corriger_app_id_groupe=$corriger_app_id_groupe<br />";
//echo "\$corriger_app_id_aid=$corriger_app_id_aid<br />";

if(($corriger_app_login_eleve=='')||(
		(($corriger_app_id_groupe=='')||(!is_numeric($corriger_app_id_groupe))||($corriger_app_id_groupe==0))&&
		(($corriger_app_id_aid=='')||(!is_numeric($corriger_app_id_aid))||($corriger_app_id_aid==0))
	)||($app=='')) {
...
if($_SESSION['statut']=='professeur') {
	...
	if(($corriger_app_id_groupe!='')&&($corriger_app_id_groupe!='0')) {
		...
		$current_aid=get_tab_aid($corriger_app_id_aid);
		if(check_prof_aid($_SESSION['login'], $corriger_app_id_aid)) {
			$envoi_mail_correction_autrui="n";

			if($current_aid["classe"]["ver_periode"][$corriger_app_id_classe][$corriger_app_num_periode]=="P") {
				// On vérifie quand même si le prof est PP avec droit de correction
				if((getSettingAOui('PeutAutoriserPPaCorrigerSesApp'))&&(acces_correction_app_aid_pp($corriger_app_id_aid))) {
					$pp_avec_droit_modif="y";
				}
			}
			$poursuivre="y";
			...
			if($current_aid["classe"]["ver_periode"][$corriger_app_id_classe][$corriger_app_num_periode]!="O") {
				if((getSettingAOui('PeutAutoriserPPaCorrigerSesApp'))&&(acces_correction_app_aid_pp($corriger_app_id_aid))) {
					$pp_avec_droit_modif="y";
					$poursuivre="y";
				}
...
".civ_nom_prenom($_SESSION['login']);
	...
	$current_aid=get_tab_aid($corriger_app_id_aid);
	$indice_aid=$current_aid['indice_aid'];

	// La période est-elle ouverte?
	if (in_array($corriger_app_login_eleve, $current_aid["eleves"][$corriger_app_num_periode]["list"])) {
		$eleve_id_classe = $current_aid["classes"]["classes"][$current_aid["eleves"][$corriger_app_num_periode]["users"][$corriger_app_login_eleve]["classe"]]["id"];

		$valider_modif="n";
		$proposer_modif="n";

		if($current_aid["classe"]["ver_periode"][$eleve_id_classe][$corriger_app_num_periode]=="N") {
			$valider_modif="y";
		}
		elseif($current_aid["classe"]["ver_periode"][$eleve_id_classe][$corriger_app_num_periode]=="P") {
			// Tester s'il y a un accès exceptionnel
			...
			elseif($pp_avec_droit_modif=="y") {
				...
				$sql="SELECT UNIX_TIMESTAMP(date_limite) AS date_limite, mode FROM matieres_app_delais WHERE id_aid='".$corriger_app_id_aid."' AND periode='$corriger_app_num_periode';";
				$res_mad=mysqli_query($mysqli, $sql);
				if($res_mad->num_rows>0) {
					$lig_mad=$res_mad->fetch_object();
					$date_limite=$lig_mad->date_limite;
		...
		if($valider_modif=="y") {
			...
			if($envoi_mail_correction_autrui=="y") {
				$ancienne_app="";
				$sql="SELECT appreciation FROM aid_appreciations WHERE (login='$corriger_app_login_eleve' AND id_aid='".$current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
				//echo "$sql<br />";
				$res=mysqli_query($GLOBALS["mysqli"], $sql);
				if(mysqli_num_rows($res)>0) {
					$lig=mysqli_fetch_object($res);
				...
				$texte_mail="Je viens d'effectuer (".strftime("%d/%m/%Y à %H:%M:%S").") une correction de votre appréciation pour ".get_nom_prenom_eleve($corriger_app_login_eleve)." en ".$current_aid["nom"]." (".$current_aid["nom_complet"]." en ".$current_aid["classlist_string"].") en période ".$corriger_app_num_periode.".
...
".civ_nom_prenom($_SESSION['login']);
				...
				//if($envoi_mail_actif=='y') {
					...
					$tab_login_destinataire=array();
					$sql="SELECT DISTINCT u.login, u.email FROM utilisateurs u, j_aid_utilisateurs jau WHERE jau.id_utilisateur=u.login AND jau.id_aid='$corriger_app_id_aid';";
					//echo "$sql<br />";
					$req=mysqli_query($mysqli, $sql);
					if(mysqli_num_rows($req)>0) {
						//$tab_email_destinataire=array();
			...
			//=========================
			// Ménage: pour ne pas laisser une demande de validation de correction alors qu'on a rouvert la période en saisie... on risquerait d'écraser par la suite l'enregistrement fait après la rouverture de période.
			$sql="DELETE FROM matieres_app_corrections WHERE (login='$corriger_app_login_eleve' AND id_aid='".$current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
			//echo "$sql<br />";
			$del=mysqli_query($GLOBALS["mysqli"], $sql);
			//=========================

			$sql="SELECT * FROM aid_appreciations WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode')";
			//echo "$sql<br />";
			$test_eleve_app_query = mysqli_query($GLOBALS["mysqli"], $sql);
			$test = mysqli_num_rows($test_eleve_app_query);
			if ($test != "0") {
				if ($app != "") {
					$sql="UPDATE aid_appreciations SET appreciation='" . $app . "' WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
				} else {
					$sql="DELETE FROM aid_appreciations WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
				}
				//echo "$sql<br />";
				...
				if ($app != "") {
					$sql="INSERT INTO aid_appreciations SET login='$corriger_app_login_eleve',id_aid='" . $current_aid["id_aid"]."', indice_aid='" . $current_aid["indice_aid"]."', periode='$corriger_app_num_periode',appreciation='" . $app . "';";
					//echo "$sql<br />";
					$register = mysqli_query($GLOBALS["mysqli"], $sql);
		...
		elseif($proposer_modif=="y") {
			...
			$sql="SELECT * FROM matieres_app_corrections WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
			//echo "$sql<br />";
			fich_debug_proposition_correction_app($prefixe_debug." : $sql\n");
			$test_eleve_app_query = mysqli_query($mysqli, $sql);
			$test = mysqli_num_rows($test_eleve_app_query);
			if ($test != "0") {
				if ($app != "") {
					$sql="UPDATE matieres_app_corrections SET appreciation='" . $app . "' WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
				} else {
					$sql="DELETE FROM matieres_app_corrections WHERE (login='$corriger_app_login_eleve' AND id_aid='" . $current_aid["id_aid"]."' AND periode='$corriger_app_num_periode');";
				}
				//echo "$sql<br />";
				...
				if (!$register) {
					...
					if ($test != "0") {
						$texte_mail="Une correction a été proposée par ".casse_mot($_SESSION['prenom'],'majf2')." ".casse_mot($_SESSION['nom'],'maj')."\r\npour l'élève ".civ_nom_prenom($corriger_app_login_eleve)." sur la période $corriger_app_num_periode\r\nen ".$current_aid['nom']." (".$current_aid["nom_complet"]." en ".$current_aid["classlist_string"].").\r\n\r\nVous pouvez valider ou rejeter la proposition en vous connectant avec un compte de statut scolarité ou secours.\r\nVous trouverez en page d'accueil, dans la rubrique Saisie, un message en rouge concernant la Correction de bulletins.\r\n";
					}
					else {
						$texte_mail="Suppression de la proposition de correction pour l'élève ".civ_nom_prenom($corriger_app_login_eleve)."\r\nsur la période $corriger_app_num_periode en ".$current_aid['nom']." (".$current_aid["nom_complet"]." en ".$current_aid["classlist_string"].")\r\npar ".casse_mot($_SESSION['prenom'],'majf2')." ".casse_mot($_SESSION['nom'],'maj').".\n";
					}
					fich_debug_proposition_correction_app($prefixe_debug." : Texte du mail:\n$texte_mail\n");
					envoi_mail_proposition_correction($corriger_app_login_eleve, $corriger_app_id_aid, $corriger_app_num_periode, $texte_mail);
					die();
				}

			} else {
				if ($app != "") {
					$sql="INSERT INTO matieres_app_corrections SET login='$corriger_app_login_eleve', id_aid='" . $current_aid["id_aid"]."', indice_aid='" . $current_aid["indice_aid"]."', periode='$corriger_app_num_periode', appreciation='" . $app . "';";
					//echo "$sql<br />";
					fich_debug_proposition_correction_app($prefixe_debug." : $sql\n");
					$register = mysqli_query($mysqli, $sql);
