...
/*$out_html = "<form id=\"matiere\"  method=\"post\" action=\"".$_SERVER['PHP_SELF']."\">
  <p> \n<label for=\"enseignement\"><strong><em>MatiËre :<br /></em></strong></label> <select id=\"enseignement\" name=\"matiere\" onchange=\"matiere_go()\">\n ";*/
...
function make_eleve_select_html($link, $login_resp, $current, $year, $month, $day)
{
	...
	// correction W3C : onChange = onchange + ajout de balise <p> +fermeture balise <option>
	  $out_html = "<form id=\"eleve\" name=\"eleve\" method=\"post\" action=\"".$_SERVER['PHP_SELF']."\"><p><label for=\"choix_eleve\"><strong><em>ElËve :</em></strong></label><br />
...
function remplace_accents($chaine,$mode){
	...
	elseif($mode == 'all_nospace'){
		...
		$retour1 = strtr(ereg_replace("∆","AE",ereg_replace("Ê","ae",ereg_replace("º","OE",ereg_replace("Ω","oe","$chaine")))),"'¬ƒ¿¡√ƒ≈« À»…ŒœÃÕ—‘÷“”’¶€‹Ÿ⁄›æ¥·‡‚‰„ÂÁÈËÍÎÓÔÏÌÒÙˆÚÛı®˚¸˘˙˝ˇ∏"," AAAAAAACEEEEIIIINOOOOOSUUUUYYZaaaaaaceeeeiiiinoooooosuuuuyyz");
...
function retourneUri($eleve, $https, $type){
	...
	if ($test_table >= 1) {
		...
		return 'Demandez ‡ votre administrateur de gÈnÈrer les URI.';
	}
