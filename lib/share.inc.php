 ...
 * 
 * $Id: share.inc.php 8577 2011-10-30 10:53:54Z crob $
 * 
...
function verif_mot_de_passe($password,$flag) {
	...
	if ($flag == 1) {
		if(preg_match("/(^[a-zA-Z]*$)|(^[0-9]*$)/", $password)) {
			$info_verif_mot_de_passe="Le mot de passe ne doit pas Ítre uniquement numÈrique ou uniquement alphabÈtique.";
			return FALSE;
		}
		elseif(preg_match("/^[[:alnum:]\W]{".getSettingValue("longmin_pwd").",}$/", $password) and preg_match("/[\W]+/", $password) and preg_match("/[0-9]+/", $password)) {
			...
			if(preg_match("/^[A-Za-z0-9]*$/", $password)) {
				$info_verif_mot_de_passe="Le mot de passe doit comporter au moins un caractËre spÈcial (#, *,...).";
			}
			elseif (strlen($password) < getSettingValue("longmin_pwd")) {
				$info_verif_mot_de_passe="La longueur du mot de passe doit Ítre supÈrieure ou Ègale ‡ ".getSettingValue("longmin_pwd").".";
				return FALSE;
			}
			else {
 ...
 * 
 * name9_p          ‡ partir du nom, rÈduit ‡ 9 caractËres + _ + premiËre lettre du prÈnom (format historique du login ÈlËve dans Gepi)
 * 
 ...
 * 
 * lcs              premiËre lettre du prÈnom + premier nom (+ _ + deuxiËme nom si le 1er nom fait moins de 4 caractËres)
 * 
 ...
 * @param string $_casse La casse du login ('maj', 'min', '') par dÈfaut la casse n'est pas modifiÈe
...
function generate_unique_login($_nom, $_prenom, $_mode, $_casse='') {
	...
	// On gÈnËre le login
	//$_prenom = strtr($_prenom, "ÁÈËÎÍ…»À ¸˚˘‹€ÔÓœŒ‰‚‡ƒ¬¿Ùˆ‘÷«", "ceeeeEEEEuuuUUiiIIaaaAAAooOOC");
	...
	//$_nom = strtr($_nom, "ÁÈËÎÍ…»À ¸˚˘‹€ÔÓœŒ‰‚‡ƒ¬¿Ùˆ‘÷«", "ceeeeEEEEuuuUUiiIIaaaAAAooOOC");
	...
	if ($_mode == "name") {
		...
		// Format d'origine des comptes ÈlËves dans Gepi
		$temp1 = $_nom;
		$temp1 = preg_replace("/ /","", $temp1);
		$temp1 = preg_replace("/-/","", $temp1);
		$temp1 = preg_replace("/'/","", $temp1);
	...
	if(count($fichier_sup)==0) {
	  ...
	  foreach ($fichier_sup as $fic_efface) {
		if(file_exists($fic_efface)) {
		  ...
		  if(file_exists($fic_efface)) {
			$nb_erreurs++;
			$erreurs.="Le fichier  <strong>".$fic_efface."</strong> n'a pas pu Ítre effacÈ.<br />";
		  }
		}
	  }
	  unset ($fic_efface);
	  if ($nb_erreurs>0) {
		if ($nb_erreurs>10) return $nb_erreurs." fichiers n'ont pu Ítre effacÈs.";
			else return $erreurs;
	  }
		else return ("Le dossier <strong>".$folder."</strong> a ÈtÈ vidÈ.") ;
	}
  }else{
...
function telecharge_fichier($sav_file,$dirname,$ext="",$type=""){
  ...
  //} else if ($sav_file['type']!=$type ){
	...
	return ("Erreur : seuls les fichiers de type '".$type."' sont autorisÈs<br />Votre fichier est de type ".$sav_file['type']);
  } else {
