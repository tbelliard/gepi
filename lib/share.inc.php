 ...
 * @param text $ajout_headers Text ‡ ajouter dans le header
 */
 ...
 * @return string|bool Le login gÈnÈrÈ ou FALSE si on obtient un login vide
 ...
 *
 * @param text $s le texte ‡ convertir
 ...
 * @return bool TRUE s'il faut faire la mise ‡ jour
 */
 ...
 *
 * @global text
 * @return bool TRUE si tout c'est bien passÈ 
 * @see getSettingValue()
 * @see saveSetting()
 */
...
function remplace_accents($chaine,$mode=''){
	...
	elseif($mode == 'all_nospace'){
		...
		//$retour1 = strtr(preg_replace("/∆/","AE",preg_replace("/Ê/","ae",preg_replace("/º/","OE",preg_replace("/Ω/","oe","$chaine")))),"'¬ƒ¿¡√ƒ≈« À»…ŒœÃÕ—‘÷“”’¶€‹Ÿ⁄›æ¥·‡‚‰„ÂÁÈËÍÎÓÔÏÌÒÙˆÚÛı®˚¸˘˙˝ˇ∏"," AAAAAAACEEEEIIIINOOOOOSUUUUYYZaaaaaaceeeeiiiinoooooosuuuuyyz");
 ...
 * @var entier $id_classe identifiant de la classe (si vide, on teste juste si le prof est PP (Èventuellement pour un ÈlËve particulier si login_eleve est non vide))
 * @var string $login_eleve login de l'ÈlËve ‡ tester (si vide, on teste juste si le prof est PP (Èventuellement pour la classe si id_classe est non vide))
 *
 * @return boolean TRUE/FALSE si l'utilisateur est PP avec les paramËtres choisis
 *
 *
 */
...
function affiche_date_sortie($date_sortie) {
	//affiche sous la forme JJ/MM/AAAA la date de sortie d'un ÈlËve prÈsente dans la base comme un timestamp
	...
	if(mysql_num_rows($res)>0) {
		...
		if((!preg_match("#^documents/acces_cdt_#",$chemin))||(strstr($chemin,".."))) {
			...
			if(!$suppr) {
				...
				$del=mysql_query($sql);
				if(!$del) {
					echo "<p><span style='color:red'>Erreur lors de la suppression des groupes associÈs ‡ l'accËs n∞$id_acces</span></p>";
					return FALSE;
				}
				else {
					...
					$del=mysql_query($sql);
					if(!$del) {
						echo "<p><span style='color:red'>Erreur lors de la suppression de l'accËs n∞$id_acces</span></p>";
						return FALSE;
					}
					else {
						return TRUE;
...
// Fonction destinÈe ‡ prendre une date mysql aaaa-mm-jj HH:MM:SS et ‡ retourner une date au format jj/mm/aaaa
function get_date_slash_from_mysql_date($mysql_date) {
	...
	if(isset($tmp_tab[0])) {
		...
		if(isset($tmp_tab2[2])) {
			...
			return "Date '".$tmp_tab[0]."' mal formatÈe?";
		}
	}
	else {
		return "Date '$mysql_date' mal formatÈe?";
	}
}
...
function get_heure_2pt_minute_from_mysql_date($mysql_date) {
	...
	if(isset($tmp_tab[1])) {
		...
		if(isset($tmp_tab2[1])) {
			...
			return "Heure '".$tmp_tab[1]."' mal formatÈe?";
		}
	}
	else {
		return "Date '$mysql_date' mal formatÈe?";
...
function message_accueil_utilisateur($login_destinataire,$texte,$date_debut=0,$date_fin=0,$date_decompte=0)
{
	...
	//	$texte : texte du message contenant Èventuellement des balises HTML et codÈ en iso-8859-1  (obligatoire)
	//	$date_debut : date ‡ partir de laquelle est affichÈ le message (timestamp, optionnel)
	//	$date_fin : date ‡ laquelle le message n'est plus affichÈ (timestamp, optionnel)
	//	$date_decompte : date butoir du dÈcompte, la chaÓne _DECOMPTE_ dans $texte est remplacÈe par un dÈcompte (timestamp, optionnel)
	
	// Retour :
	// TRUE ou FALSE selon que le massage a ÈtÈ enregistrÈ ou pas
...
function nb_saisies_bulletin($type, $id_groupe, $periode_num, $mode="") {
	...
	if($mode=="couleur") {
		if($nb_saisies_bulletin==$effectif_groupe){
			$retour="<span style='font-size: x-small;' title='Saisies complËtes'>";
			$retour.="($nb_saisies_bulletin/$effectif_groupe)";
			$retour.="</span>";
		}
		else {
			$retour="<span style='font-size: x-small; background-color: orangered;' title='Saisies incomplËtes ou non encore effectuÈes'>";
			$retour.="($nb_saisies_bulletin/$effectif_groupe)";
			$retour.="</span>";
		}
	}
