...
function generate_unique_login($_nom, $_prenom, $_mode, $_casse='') {
	...
	while ($test1 == "_" OR $test1 == "-" OR $test1 == ".") {
		$login_user = mb_substr($login_user, 0, strlen($login_user)-1);
		$test1 = $login_user{strlen($login_user)-1};
	}
...
function checkAccess() {
    ...
    if ($_SESSION["statut"] == 'autre') {
    	...
    	where nom_fichier = '" . mb_substr($url['path'], strlen($gepiPath)) . "'
	    ...
	    from droits
    	where id = '" . mb_substr($url['path'], strlen($gepiPath)) . "'
    	;";

	}

    $dbCheckAccess = sql_query1($sql);
    if (mb_substr($url['path'], 0, strlen($gepiPath)) != $gepiPath) {
...
function tentative_intrusion($_niveau, $_description) {
    ...
    $fichier = mb_substr($url['path'], strlen($gepiPath));
...
function casse_mot($mot,$mode='maj') {
    ...
    if (function_exists('mb_convert_case')) {
    	...
    	elseif($mode=='majf') {
    		if(strlen($str)>1) {
    			return strtoupper(mb_substr($str,0,1)).strtolower(mb_substr($str,1));
    		}
    		else {
    			return strtoupper($str);
    		}
