...
function cree_variables_non_protegees() {
    ...
    foreach ($_POST as $key => $value) {
        if (mb_substr($key,0,15) == "no_anti_inject_") {
            $temp = mb_substr($key,15,strlen($key));
...
if ((!(in_array(mb_substr($url['path'], strlen($gepiPath)),$liste_scripts_non_traites))) OR ((in_array(mb_substr($url['path'], strlen($gepiPath)),$liste_scripts_non_traites)) AND (!(isset($traite_anti_inject)) OR (isset($traite_anti_inject) AND $traite_anti_inject !="no")))) {
