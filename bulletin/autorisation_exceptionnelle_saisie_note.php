							...
							if(!$res) {
								...
								if($envoi_mail_actif=='y') {
									...
									if(mysqli_num_rows($req)>0) {
										...
										$texte_mail="Vous avez jusqu'au $date_limite_email pour saisir/corriger une ou des notes de bulletins pour l'AID ".$current_aid['nom']." (".$current_aid['nom_complet']." en ".$current_aid['classlist_string'].") en période $periode.\n\n";
										$message_autorisation_exceptionnelle_bull_note=getSettingValue('message_autorisation_exceptionnelle_bull_note');
...
elseif(
		...
		((!isset($enseignement_periode)&&(!isset($aid_periode))))
	) {
	...
	<fieldset class='fieldset_opacite50'>
		<h2>Autoriser la modification de moyennes des bulletins en ".$classe."</h2>
	...
	if(count($tab_aid_clas)>0) {
		...
		foreach($tab_aid_clas as $current_aid) {
			if(($current_aid["display_bulletin"]=="y")&&($current_aid["type_note"]!='no')) {
				...
				for($i=1;$i<$nb_periode;$i++) {
					if($ver_periode[$i]=='O') {
						$lignes_aid.="<td><img src='../images/disabled.png' width='20' height='20' alt='Période $i close' title='Période non concernée par la notation' /></td>\n";
					}
					elseif(($current_aid["type_note"]!='every')&&
					(($i<$current_aid["display_begin"])||($i>$current_aid["display_end"])||
					(($current_aid["type_note"]=='last')&&($i!=$current_aid["display_end"]))
					)) {
						$lignes_aid.="<td><img src='../images/disabled.png' width='20' height='20' alt='Période $i close' title='Période $i close' /></td>\n";
					}
					elseif($ver_periode[$i]=='P') {
						...
						$sql="SELECT UNIX_TIMESTAMP(date_limite) AS date_limite FROM acces_exceptionnel_matieres_notes WHERE id_aid='".$current_aid['id_aid']."' AND periode='$i';";
						$res=mysqli_query($GLOBALS["mysqli"], $sql);
						if(mysqli_num_rows($res)>0) {
							$lig=mysqli_fetch_object($res);
							if($lig->date_limite>$date_courante) {
...
elseif((isset($id_groupe))&&(isset($periode))) {
	...
	if(($_SESSION['statut']=='administrateur')||(($_SESSION['statut']=='scolarite')&&(getSettingAOui('PeutDonnerAccesBullAppPeriodeCloseScol')))) {
		...
		echo "<div style='margin-left:3em;'>";
			echo "<p>La saisie/correction de notes, si elle est autorisée ici, n'est pas limitée.<br />
			En revanche, il est possible de soumettre à vérification la correction d'appréciations&nbsp;:</p>";
...
elseif((isset($id_aid))&&(isset($periode))) {
	...
	echo "<p>Vous souhaitez autoriser exceptionnellement un enseignant à effectuer des saisies/corrections de notes de bulletins pour l'AID <strong>".$aid['nom']." (<span style='font-size:x-small;'>".$aid['nom_complet']." en ".$aid['classlist_string']." avec ".$aid['proflist_string']."</span>)</strong> en <strong>période $periode</strong>.</p>\n";

	$sql="SELECT commentaires, UNIX_TIMESTAMP(date_limite) AS date_limite FROM acces_exceptionnel_matieres_notes WHERE id_aid='".$aid['id_aid']."' AND periode='$periode';";
	$res=mysqli_query($GLOBALS["mysqli"], $sql);
	if(mysqli_num_rows($res)>0) {
		$lig=mysqli_fetch_object($res);
		$date_limite=$lig->date_limite;
		...
		//$tab_date_limite=get_date($date_limite);
		echo "<p class='bold' style='color:blue'>Une autorisation exceptionnelle de saisie existe pour cet AID/période&nbsp;: ".strftime("%d/%m/%Y à %H:%M",$date_limite)."</p>\n";
		$display_date_limite=strftime("%d/%m/%Y",$date_limite);
		$display_heure_limite=strftime("%H:%M",$date_limite);
	...
	echo "<input type='hidden' name='is_posted' value='y' />\n";
	echo "<input type='hidden' name='id_classe' value='$id_classe' />\n";
	echo "<input type='hidden' name='aid_periode[]' value='$id_aid|$periode' />\n";
	echo "<input type='text' name = 'display_date_limite' id = 'display_date_limite' size='8' value = \"".$display_date_limite."\" onKeyDown=\"clavier_date(this.id,event);\" AutoComplete=\"off\" />\n";
	echo img_calendrier_js("display_date_limite", "img_bouton_display_date_limite");
