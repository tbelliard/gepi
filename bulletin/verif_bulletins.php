...
if (!(isset($id_classe))) {
	...
	if ($_SESSION["statut"] == "scolarite") {
		...
		$appel_donnees = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT DISTINCT c.* FROM classes c, j_eleves_professeurs s, j_eleves_classes cc WHERE (s.professeur='" . $_SESSION['login'] . "' AND s.login = cc.login AND cc.id_classe = c.id)");
	}
	...
	*/
	...
	$test1 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT distinct(login) login from j_eleves_classes WHERE id_classe='" . $id_classe . "'");
	$nb_eleves = mysqli_num_rows($test1);
	$j = 0;
	$flag = true;
	while ($j < $nb_eleves) {
		$login_e = mysql_result($test1, $j, "login");
		$test = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM j_eleves_cpe WHERE e_login='" . $login_e . "'"), 0);
		if ($test == "0") {
			$flag = false;
			break;
		}
	...
	*/
	...
	$appel_donnees_eleves = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT e.login, e.nom, e.prenom FROM eleves e, j_eleves_classes j WHERE (j.id_classe='$id_classe' AND j.login = e.login and j.periode='$per') ORDER BY login");
	...
	while($j < $nb_eleves) {
		...
		if(($mode=="note_app")||($mode=="tout")||($mode=="tout_sauf_avis")) {
			$groupeinfo = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT DISTINCT id_groupe FROM j_eleves_groupes WHERE login='" . $id_eleve[$j] ."'");
			...
			while($i < $lignes_groupes) {
				...
				if (((!isset($current_group['visibilite']['bulletins']))||($current_group['visibilite']['bulletins']!='n'))&&(in_array($id_eleve[$j], $current_group["eleves"][$per]["list"]))) { // Si l'élève suit cet enseignement pour la période considérée
					...
					//Vérification des appréciations :
					//
					$test_app = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM matieres_appreciations WHERE (login = '$id_eleve[$j]' and id_groupe = '" . $current_group["id"] . "' and periode = '$per')");
			...
			while($i < $lignes_groupes) {
				...
				if (((!isset($current_group['visibilite']['bulletins']))||($current_group['visibilite']['bulletins']!='n'))&&(in_array($id_eleve[$j], $current_group["eleves"][$per]["list"]))) { // Si l'élève suit cet enseignement pour la période considérée
					...
					//Vérification des moyennes :
					//
					$test_notes = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM matieres_notes WHERE (login = '$id_eleve[$j]' and id_groupe = '" . $current_group["id"] . "' and periode = '$per')");
		...
		if(($mode=="avis")||($mode=="tout")) {
			...
			//
			$query_conseil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM avis_conseil_classe WHERE (login = '$id_eleve[$j]' and periode = '$per')");
			...
			if ($avis == '') {
				...
				$call_prof = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT u.login, u.nom, u.prenom FROM utilisateurs u, j_eleves_professeurs j WHERE (j.login = '$id_eleve[$j]' and j.id_classe='$id_classe' and u.login=j.professeur)");
		...
		if(($mode=="note_app")||($mode=="tout")||($mode=="tout_sauf_avis")) {
			...
			//
			$call_data = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM aid_config WHERE display_bulletin!='n' ORDER BY nom");
			...
			while ($z < $nb_aid) {
				...
				if (($per >= $display_begin) and ($per <= $display_end)) {
					...
					$call_data2 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM aid_config WHERE indice_aid = '$indice_aid'");
					$nom_aid = @mysql_result($call_data2, 0, "nom");
					$aid_query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id_aid FROM j_aid_eleves WHERE (login='$id_eleve[$j]' and indice_aid='$indice_aid')");
					$aid_id = @mysql_result($aid_query, 0, "id_aid");
					if ($aid_id != '') {
						$aid_app_query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM aid_appreciations WHERE (login='$id_eleve[$j]' AND periode='$per' and id_aid='$aid_id' and indice_aid='$indice_aid')");
						$query_resp = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT u.login, u.nom, u.prenom FROM utilisateurs u, j_aid_utilisateurs j WHERE (j.id_aid = '$aid_id' and u.login = j.id_utilisateur and j.indice_aid='$indice_aid')");
		...
		if(($mode=="abs")||($mode=="tout")||($mode=="tout_sauf_avis")) {
			...
			//
			$abs_query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM absences WHERE (login='$id_eleve[$j]' AND periode='$per')");
			...
			if (($abs1 == '') or ($abs2 == '') or ($abs3 == '')) {
				...
				$query_resp = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT u.login, u.nom, u.prenom FROM utilisateurs u, j_eleves_cpe j WHERE (j.e_login = '$id_eleve[$j]' AND u.login = j.cpe_login)");
				$nb_prof = mysqli_num_rows($query_resp);
				$m=0;
				$virgule = 1;
				while ($m < $nb_prof) {
		...
		if($gepiSettings['active_mod_ects'] == 'y' && (($mode=="ects")||($mode=="tout")||($mode=="tout_sauf_avis"))) {
            ...
            $query_groupes_ects = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT jgc.* FROM j_groupes_classes jgc, j_eleves_groupes jeg WHERE jgc.saisie_ects = '1' AND jgc.id_classe = '$id_classe' AND jgc.id_groupe = jeg.id_groupe AND jeg.login = '".$id_eleve[$j]."' AND jeg.periode = '$per'");
            if (mysqli_num_rows($query_groupes_ects) > 0) {
                $temoin_has_ects = true;
                $query_conseil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT ec.* FROM ects_credits ec, eleves e WHERE ec.id_eleve = e.id_eleve AND e.login = '$id_eleve[$j]' AND num_periode = '$per'");
