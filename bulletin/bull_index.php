 ...
 *Il faudrait peut-Ãªtre revoir le dispositif pour adopter la mÃªme stratégie.
...
// Pour sur le verso du bulletin n'avoir qu'un relevé de notes et pas deux... et surtout pas celui de l'élÃ¨ve suivant dans la liste:
...
mais il se peut que vous ayez ainsi des précisions sur ce qui pose problÃ¨me.<br />
</p>\n";
	}
...
if(!isset($tab_id_classe)) {
	...
	(($_SESSION['statut']=='administrateur')&&(getSettingValue('GepiAdminImprBulSettings')=='yes'))) {
		echo " | <a href='param_bull.php' target='_blank'>ParamÃ¨tres d'impression des bulletins</a>";
	}
	...
	if ($_SESSION["statut"] == "scolarite") {
		...
		$message_0="Aucune classe (<i>avec élÃ¨ve</i>) ne vous est affectée.";
	}
	...
	elseif (($_SESSION["statut"] == "administrateur")||($_SESSION["statut"] == "secours")) {
		...
		$message_0="Aucune classe avec élÃ¨ve n'a été trouvée.";
	}
	elseif ($_SESSION["statut"] == "professeur") {
		$sql="SELECT DISTINCT c.* FROM classes c, j_eleves_professeurs jep, j_eleves_classes jec WHERE (jep.professeur='".$_SESSION['login']."' AND jep.login = jec.login AND jec.id_classe = c.id);";
			...
			$message_0="Aucune classe (<i>avec élÃ¨ve</i>) ne vous est affectée pour l'édition des bulletins.";
	}
	else {
	...
	if($nb_classes==0){
		//echo "<p>Aucune classe avec élÃ¨ve affecté n'a été trouvée.</p>\n";
		echo "<p>".$message_0."</p>\n";
...
elseif(!isset($_POST['valide_select_eleves'])) {
	...
	//echo "<p class='bold'>Sélection des élÃ¨ves:</p>\n";
	echo "<p class='bold'>Sélection des élÃ¨ves et paramÃ¨tres:</p>\n";

	echo "\n<!-- Formulaire de sélection des élÃ¨ves et de paramétrage -->\n";
	...
	if ($test == -1) {
		...
		if($_SESSION['statut']=='administrateur') {
			echo "Visitez la page de <a href='test_modele_bull.php'>création de cette table</a> d'aprÃ¨s l'ancienne table 'model_bulletin' pour permettre l'impression de bulletins PDF.";
		}
		else {
			echo "Le remplissage de la table doit Ãªtre effectué en admin.";
		}
	}
	else {
		...
		if(mysql_num_rows($test)==0) {
			...
			echo "<div id='div_modele_bulletin_pdf'>\n";
				...
				if (($option_modele_bulletin==2)||($option_modele_bulletin==3)) { //Par défaut  le modÃ¨le défini pour les classes
					echo "Choisir le modÃ¨le de bulletin<br />\n";
					// sélection des modÃ¨les des bulletins PDF
					...
					$option_modele_bulletin=getSettingValue("option_modele_bulletin");
					if ($option_modele_bulletin==2) { //Par défaut  le modÃ¨le défini pour les classes
						echo "<option value=\"-1\">Utiliser les modÃ¨les pré-sélectionnés par classe</option>\n";
					}
			...
			$texte_infobulle="Il arrive que la génération de PDF échoue.<br />Les raisons peuvent Ãªtre variables (<em>manque de ressources serveur, bug,...</em>).<br />Dans ce cas, la présence d'un plugin lecteur PDF peut empÃªcher de voir quelles erreurs provoquent l'échec.<br />En cochant la case DEBUG, vous obtiendrez l'affichage des erreurs et ainsi vous pourrez obtenir de l'aide plus facilement sur la liste 'gepi-users'<br />\n";
			//$texte_infobulle.="\n";
	...
	//if($test_coef>0){
		...
		echo ".</b></td>\n";
		echo "</tr>\n";
		echo "<tr><td valign='top'><input type='checkbox' name='coefficients_a_1' id='coefficients_a_1' value='oui' /></td><td><label for='coefficients_a_1' style='cursor: pointer;'>Forcer les coefficients des matiÃ¨res Ã  1, indépendamment des coefficients saisis dans les paramÃ¨tres de la classe.</label></td></tr>\n";
		*/
		echo "<tr><td valign='top'><input type='checkbox' name='coefficients_a_1' id='coefficients_a_1' value='oui' onchange=\"checkbox_change(this.id)\" /></td><td><label for='coefficients_a_1' id='texte_coefficients_a_1' style='cursor: pointer;'>Forcer, dans le calcul des moyennes générales, les coefficients des matiÃ¨res Ã  1, indépendamment des coefficients saisis dans les paramÃ¨tres de la classe.</label></td></tr>\n";

		// A FAIRE: A déplacer et mettre dans le modÃ¨le PDF
		//echo "<tr id='id_tr_moyennes_periodes_precedentes'><td valign='top'><input type='checkbox' name='moyennes_periodes_precedentes' id='moyennes_periodes_precedentes' value='y'  /></td><td><label for='moyennes_periodes_precedentes' style='cursor: pointer;'>Afficher les moyennes de l'élÃ¨ve pour les périodes précédentes (<i>incompatible avec l'affichage des moyennes min/max/classe dans la mÃªme cellule que la moyenne de l'élÃ¨ve</i>).</label></td></tr>\n";
		...
		/*
		echo "<tr><td valign='top'><input type='checkbox' name='avec_coches_mentions' id='avec_coches_mentions' value='y' /></td><td><label for='avec_coches_mentions' style='cursor: pointer;'>Faire apparaÃ®tre des cases Ã  cocher pour les ".$gepi_denom_mention."s (<i>Félicitations, Mention honorable, Encouragements, ou autre...</i>) sur les bulletins PDF.</label></td></tr>\n";
		*/
	...
	// L'admin peut avoir accÃ¨s aux bulletins, mais il n'a de toute faÃ§on pas accÃ¨s au relevés de notes.
	...
	if (mysql_num_rows($res_verif_droit)>0) {
		...
		echo " les paramÃ¨tres du relevé de notes</i>).";
		echo "</span>\n";
...
";
	...
	*/
	...
	if(count($tab_id_classe)>1) {
		...
		echo " <a href='#' onClick='cocher_tous_eleves();return false;'>Cocher tous les élÃ¨ves</a> / <a href='#' onClick='decocher_tous_eleves();return false;'>Décocher tous les élÃ¨ves</a></p>\n";
	}
	...
	for($i=0;$i<count($tab_id_classe);$i++) {
		...
		echo "<table class='boireaus' summary='Choix des élÃ¨ves'>\n";
		echo "<tr>\n";
		echo "<th>ElÃ¨ves</th>\n";
		for($j=0;$j<count($tab_periode_num);$j++) {
			// Est-ce bien un entier?
			...
			// En imprimant deux classes l'une Ã  semestres, l'autre Ã  trimestres, on peut choisir la période 3 et provoquer des erreurs sur les semestres...
...
function display_param_releve() {
	...
	// On donne l'accÃ¨s aux liens de pliage/dépliage des paramÃ¨tres du relevé de notes
...
// On cache l'accÃ¨s aux liens de pliage/dépliage des paramÃ¨tres du relevé de notes
// jusqu'Ã  ce que la case d'insertion des relevés de notes entre les bulletins ait été cochée (au moins une fois)
...
//==EXTRACTION DES DONNEES PUIS AFFICHAGE DES BULLETINS==
	...
	echo "<a href='".$_SERVER['PHP_SELF']."'>Choisir d'autres classes</a>\n";
	echo " | <a href='#' onClick='history.go(-1);'>Choisir d'autres périodes</a>\n";
	echo " | <a href='#' onClick='history.go(-2);'>Choisir d'autres élÃ¨ves</a>\n";
	echo "</p>\n";
	*/
...
<td id='td_ele'></td>
	...
	// Peut-Ãªtre déplacer Ã§a vers un fichier externe d'initialisation de variables
	...
	*/
	...
	if ($bull_affiche_aid == 'y') {
		...
		// On prépare l'affichage des appréciations des Activités Interdisciplinaires devant apparaÃ®tre en tÃªte des bulletins :
		...
		// On prépare l'affichage des appréciations des Activités Interdisciplinaires devant apparaÃ®tre en fin des bulletins :
	...
	// Tableau destiné Ã  stocker toutes les infos
	$tab_bulletin=array();
	...
	for($loop_classe=0;$loop_classe<count($tab_id_classe);$loop_classe++) {
		...
		if((isset($_POST['forcer_recalcul_rang']))&&($_POST['forcer_recalcul_rang']=='y')) {
			...
			// Les rangs seront recalculés lors de l'appel Ã  calcul_rang.inc.php
		}
...
/*
			...
			$tab_modele_pdf["entente_tel"][$tab_id_classe[$loop_classe]]='1'; // afficher le numéro de téléphone dans l'entÃªte
			$tab_modele_pdf["entente_fax"][$tab_id_classe[$loop_classe]]='1'; // afficher le numéro de fax dans l'entÃªte
			...
			$tab_modele_pdf["active_bloc_eleve"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher les informations sur l'élÃ¨ve
			$tab_modele_pdf["active_bloc_adresse_parent"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher l'adresse des parents
			$tab_modele_pdf["active_bloc_absence"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher les absences de l'élÃ¨ve
			...
			$tab_modele_pdf["active_photo"][$tab_id_classe[$loop_classe]] = '0'; // fait - afficher la photo de l'élÃ¨ve
			$tab_modele_pdf["active_coef_moyenne"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher le coéficient des moyenne par matiÃ¨re
			$active_coef_sousmoyene = '1'; // fait - afficher le coéficient des moyenne par matiÃ¨re
			$tab_modele_pdf["active_nombre_note"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher le nombre de note par matiÃ¨re sous la moyenne de l'élÃ¨ve
			...
			$tab_modele_pdf["active_moyenne_eleve"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher la moyenne de l'élÃ¨ve
			...
			$tab_modele_pdf["active_rang"][$tab_id_classe[$loop_classe]] = '1'; // fait - afficher le rang de l'élÃ¨ve
			...
			$tab_modele_pdf["affiche_doublement"][$tab_id_classe[$loop_classe]] = '1'; // affiche si l'élÃ¨ve Ã  doubler
			$tab_modele_pdf["affiche_date_naissance"][$tab_id_classe[$loop_classe]] = '1'; // affiche la date de naissance de l'élÃ¨ve
			$tab_modele_pdf["affiche_lieu_naissance"][$tab_id_classe[$loop_classe]] = '0'; // affiche le lieu de naissance de l'élÃ¨ve
			...
			$tab_modele_pdf["toute_moyenne_meme_col"][$tab_id_classe[$loop_classe]]='0'; // afficher les information moyenne classe/min/max sous la moyenne général de l'élÃ¨ve
			$active_coef_sousmoyene = '1'; //afficher le coeficent en dessous de la moyenne de l'élÃ¨ve
			...
			$tab_modele_pdf["ajout_cadre_blanc_photo"][$tab_id_classe[$loop_classe]] = '0'; // ajouter un cadre blanc pour la photo de l'élÃ¨ve.
			...
			$tab_modele_pdf["affiche_ine"][$tab_id_classe[$loop_classe]] = '0'; // affiche l'INE de l'élÃ¨ve
			...
			$tab_modele_pdf["affiche_numero_responsable"][$tab_id_classe[$loop_classe]] = '0'; // affichage du numéro du responsable legal de l'élÃ¨ve dont le bulletin est imprimé. 1 ==> affiche 0 ==> n'affiche pas
			*/
		...
		// ++++++++++++++++++++++++++++++++++++++
		// ++++++++++++++++++++++++++++++++++++++
		// AJOUTER UN TEST: Le visiteur a-t-il le droit d'accéder Ã  cette page pour cette classe
		if ($_SESSION["statut"] == "scolarite") {
		...
		if(mysql_num_rows($test_acces_classe)==0) {
            tentative_intrusion(2, "Tentative d'un ".$_SESSION["statut"]." (".$_SESSION["login"].") Ã  une classe (".get_class_from_id($id_classe).") sans y Ãªtre autorisé.");
            echo "<p>Vous n'Ãªtes pas autorisés Ã  Ãªtre ici.</p>\n";
			require("../lib/footer.inc.php");
			die();
		}
		// ++++++++++++++++++++++++++++++++++++++
	...
	document.getElementById('td_periode').innerHTML='".$periode_num."';
			...
			// Liste des élÃ¨ves Ã  éditer/afficher/imprimer (sélection):
			// tab_ele_".$i."_".$j.
			...
			//Afficher les coefficients des matiÃ¨res (uniquement si au moins un coef différent de 0)
			if($test_coef>0){
			...
			// Vérifier si il n'y a pas de bÃªtise sur les catégories.
			if($affiche_categories) {
				...
				if($nb_cat_priorites!=$nb_cat) {
					// Tester si les catégories de matiÃ¨res ont bien des priorités différentes au niveau Gestion des matiÃ¨res
					...
					if($nb_cat_priorites_glob!=$nb_cat) {
						if($mode_bulletin!="pdf") {
							...
							echo "<p>Vous avez demandé Ã  afficher les catégories de matiÃ¨res, mais les priorités d'affichage des catégories ne sont pas correctement définies, ni au niveau global dans Gestion des matiÃ¨res, ni au niveau particulier dans Gestion des classes/&lt;Classe&gt; ParamÃ¨tres<br />Il ne faut pas que deux catégories aient la mÃªme priorité sans quoi il peut survenir des anomalies d'ordre des matiÃ¨res sur le bulletin.</p>\n";
							require("../lib/footer.inc.php");
							die();
						}
						else {
							...
							$pdf->SetXY(20,40);
							$pdf->SetFontSize(10);
							$pdf->Cell(150,7, "Vous avez demandé Ã  afficher les catégories de matiÃ¨res,",0,2,'');
							$pdf->SetXY(20,45);
							...
							$pdf->SetXY(20,65);
							$pdf->Cell(150,7, "Il ne faut pas que deux catégories aient la mÃªme priorité",0,2,'');
							$pdf->SetXY(20,70);
			...
			//if($eff_classe>0) {
				include("../lib/calcul_moy_gen.inc.php");
			//}
			// On récupÃ¨re la plus grande partie des infos via calcul_moy_gen.inc.php
			...
			*/
			...
			// Variables récupérées de calcul_moy_gen.inc.php
			// Tableaux d'indices [$j][$i] (groupe, élÃ¨ve)
			// A VéRIFIER: Tableaux d'indices [$i][$j] (élÃ¨ve, groupe)
			...
			*/
			...
			// Tableaux d'indice $i (correspondant Ã  l'élÃ¨ve)
			...
			// Variables récupérées de calcul_moy_gen.inc.php
			// Tableau d'indice [$i] élÃ¨ve.. mais cette moyenne générale ne prend en compte que les options suivies par l'élÃ¨ve si bien que les moyennes générales de classe diffÃ¨rent selon les élÃ¨ves
			...
			*/
			...
			if($mode_bulletin=="pdf") {
				...
				if(($temoin_tous_coef_a_1=='y')||($coefficients_a_1=="oui")) {
					// Si tous les coeff sont Ã  1, on n'imprime pas deux lignes de moyenne générale (moy.gen.coefficientée d'aprÃ¨s Gestion des classes/<Classe> Enseignements et moy.gen avec coef Ã  1) mÃªme si la case est cochée dans le modÃ¨le PDF.
					// Si on force les coef Ã  1, on n'affiche pas non plus deux lignes de moyenne générale
				...
				// Il faut décider oÃ¹ on fait le paramétrage.
				// Les paramÃ¨tres HTML sont généraux Ã  toutes les classes sauf ceux décidés directement dans bull_index.php alors que les paramÃ¨tres PDF sont essentiellement liés aux modÃ¨les.
				$tab_bulletin[$id_classe][$periode_num]['affiche_moyenne_general_coef_1']=0;
			}
			...
			// $place_eleve_classe est un tableau d'indice [$i] le numéro de l'élÃ¨ve
			...
			// L'ordre des matiÃ¨res est obtenu via calcul_moy_gen.inc.php dans lequel le $affiche_categorie fixe l'ordre par catégories ou non.



			// Boucle élÃ¨ves de la classe $id_classe pour la période $periode_num
			for($i=0;$i<count($current_eleve_login);$i++) {
				// Réinitialisation pour ne pas récupérer des infos de l'élÃ¨ve précédent
				unset($tab_ele);
				$tab_ele=array();
				...
				if (in_array($current_eleve_login[$i],$tab_bulletin[$id_classe][$periode_num]['selection_eleves'])) {
					...
					// ++++++++++++++++++++++++++++++++++++++
					// ++++++++++++++++++++++++++++++++++++++
					// AJOUTER UN TEST: L'élÃ¨ve fait-il bien partie de la classe?
					...
					//                  On pourrait injecter un login dans la sélection d'élÃ¨ve, mais pas dans $current_eleve_login
					...
					if($mode_bulletin=="html") {
						...
						decompte_debug($motif,"$motif élÃ¨ve $i: ".$current_eleve_login[$i]);
						flush();
						echo "<script type='text/javascript'>
	document.getElementById('td_ele').innerHTML='".$current_eleve_login[$i]."';
					...
					// Récup des infos sur l'élÃ¨ve, les responsables, le PP, le CPE,...
					...
					if ($affiche_rang == 'y'){
						...
						// Rang de l'élÃ¨ve dans la classe (par rapport Ã  la moyenne générale)
					...
					if ($bull_affiche_aid == 'y') {
						...
						if($mode_bulletin=="html") {
							...
							decompte_debug($motif,"$motif élÃ¨ve $i (".$current_eleve_login[$i].") avant AID");
							flush();
						}
						//==============================

						// On attaque maintenant l'affichage des appréciations des Activités Interdisciplinaires devant apparaÃ®tre en tÃªte des bulletins :
						//------------------------------
						// $z est l'indice de $call_data_aid_b
						$z=0;
						// $zz est l'indice des AID effectivement utiles pour l'élÃ¨ve et la période
						$zz=0;
						while ($z < $nb_aid_b) {
							...
							*/
							if (($periode_num >= $display_begin) and ($periode_num <= $display_end)) {
								...
								if ($aid_id != '') {
									...
									//------
									// On appelle l'appréciation de l'élÃ¨ve, et sa note
									//------
									...
									if (($type_note == 'every') or (($type_note == 'last') and ($periode_num == $last_periode_aid))) {
										...
										if ($current_eleve_aid_note == '') {
											...
											if ($affiche_graph == 'y')  {
												...
												// Pas idéal: on fait ces requÃªtes autant de fois qu'il y a d'élÃ¨ve...
									...
									// La deuxiÃ¨me solution réduirait sans doute le nombre de requÃªtes
						...
						// On attaque maintenant l'affichage des appréciations des Activités Interdisciplinaires devant apparaÃ®tre en fin des bulletins :
						//------------------------------
						...
						if($mode_bulletin=="html") {
							$motif="Temoin_eleve".$id_classe."_".$periode_num."_".$i;
							decompte_debug($motif,"$motif élÃ¨ve $i (".$current_eleve_login[$i].") aprÃ¨s AID");
							flush();
						}
						//==============================
					}
					...
					for($j=0;$j<count($current_group);$j++) {
						//============================================
						// A REVOIR: On fait cette requÃªte autant de fois qu'il y a d'élÃ¨ve... l'extraire de la boucle élÃ¨ves en faisant une autre boucle sur $current_group
						...
						// Si l'élÃ¨ve suit l'option, sa note est affectée (éventuellement vide)
						if(isset($current_eleve_note[$j][$i])) {
							...
							*/
							...
							// On teste si des notes de une ou plusieurs boites du carnet de notes doivent Ãªtre affichée
							...
							// Pour remplacer la chaine de caractÃ¨res @@Notes par une insertion des notes dans l'appréciation du bulletin.
					...
					// On affecte la partie élÃ¨ve $tab_ele dans $tab_bulletin
	...
	document.getElementById('td_ele').innerHTML='".$tab_bulletin[$id_classe][$periode_num]['eleve'][$rg[$i]]['login']."';
							...
							if($mode_bulletin=="html") {
								...
								//$motif="Bulletin_eleve".$id_classe."_".$periode_num."_".$i;
								//decompte_debug($motif,"$motif élÃ¨ve $i aprÃ¨s");
								$motif="Bulletin_eleve".$id_classe."_".$periode_num."_".$rg[$i];
								decompte_debug($motif,"$motif élÃ¨ve $rg[$i] aprÃ¨s");
								flush();
							}
							else {
...
Il faudrait peut-Ãªtre revoir le dispositif pour adopter la mÃªme stratégie.<br />
