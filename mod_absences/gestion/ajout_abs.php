...
if($mode === 'classe')
{
	...
	$cpt_eleves = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*)
	...
	*/
	...
	$execution_eleve = mysqli_query($GLOBALS["___mysqli_ston"], $requete_eleve) or die('Erreur SQL !'.$requete_eleve.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
	$cpt_eleve = 0;
...
$query_h = mysqli_query($GLOBALS["___mysqli_ston"], $sql_h);
    ...
    while ($total < $nb_i) // $nb_i étant envoyé par le navigateur et représentant le nombre d'absences envoyées par le navigateur
    {
		...
		if($active_absence_eleve === 'oui')
		{
			...
			if ($dp_absence_eleve != "")
			{
                ...
                $resultat_recherche_periode = mysqli_query($GLOBALS["___mysqli_ston"], $requete_recherche_periode) or die('Erreur SQL !'.$requete_recherche_periode.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
			...
			$resultat_m = mysqli_query($GLOBALS["___mysqli_ston"], $requete) or die('Erreur SQL !'.$requete.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
			...
			if ($erreur === '1'){
				...
				if ( $justify_absence_eleve === 'N' and $motif_absence_eleve_ins != 'RE') {
					...
					$cpt_lettre_suivi = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*)
				...
				if ( ($justify_absence_eleve === 'O' or $justify_absence_eleve === 'T') and $motif_absence_eleve_ins != 'RE') {
					...
					$date_emis = date('Y-m-d');
					$heure_emis = date('H:i:s');
					$cpt_lettre_suivi = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."lettres_suivis WHERE quirecois_lettre_suivi = '".$eleve_absence_eleve."' AND emis_date_lettre_suivi = '".$date_emis."' AND partde_lettre_suivi = 'absences_eleves'"),0);
...
if ($action === 'supprimer')
{
    ...
    $cpt_lettre_recus = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."lettres_suivis WHERE partde_lettre_suivi = 'absences_eleves' AND type_lettre_suivi = '6' AND partdenum_lettre_suivi LIKE '%,".$id_absence_eleve.",%' AND (statu_lettre_suivi = 'recus' OR envoye_date_lettre_suivi != '0000-00-00')"),0);
    ...
    if( $cpt_lettre_recus === '0' ) {
		...
		if ( $id_absence_eleve != '' )
		{
			...
			$cpt_lettre_suivi = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."lettres_suivis WHERE partde_lettre_suivi = 'absences_eleves' AND type_lettre_suivi = '6' AND partdenum_lettre_suivi = ',".$id_absence_eleve.",'"),0);
			if( $cpt_lettre_suivi == 1 ) {
			...
			$cpt_lettre_suivi = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."lettres_suivis WHERE partde_lettre_suivi = 'absences_eleves' AND type_lettre_suivi = '6' AND partdenum_lettre_suivi LIKE '%,".$id_absence_eleve.",%'"),0);
			if( $cpt_lettre_suivi == 1 ) {
				$requete = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM ".$prefix_base."lettres_suivis WHERE partde_lettre_suivi = 'absences_eleves' AND type_lettre_suivi = '6' AND partdenum_lettre_suivi LIKE '%,".$id_absence_eleve.",%'");
...
?>"><img src="../../lib/calendrier/petit_calendrier.gif" border="0" alt="" /></a></td>
    ...
    <?php
           ...
           $resultat = mysqli_query($GLOBALS["___mysqli_ston"], $requete_t) or die('Erreur SQL !'.$requete.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
    ?>
