...
//importation des informations de présentation de la lettre type
           ...
           $execution_structure = mysqli_query($GLOBALS["___mysqli_ston"], $requete_structure) or die('Erreur SQL !'.$requete_structure.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
...
$etiquette_agencement = '3';
	...
	if($classe != 'tous') { $requete_persone ="SELECT * FROM ".$prefix_base."classes c, ".$prefix_base."j_eleves_classes ec, ".$prefix_base."eleves e WHERE c.id = '".$classe."' AND ec.id_classe = c.id AND ec.login = e.login GROUP BY e.login ".$trie.""; }
        ...
        $execution_persone = mysqli_query($GLOBALS["___mysqli_ston"], $requete_persone) or die('Erreur SQL !'.$requete_persone.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
        while ( $donne_persone = mysqli_fetch_array($execution_persone))
	 {
		...
		$nombre_de_responsable =  mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."resp_pers rp, ".$prefix_base."resp_adr ra, ".$prefix_base."responsables2 r WHERE ( r.ele_id = '".$eleve_numid[$i]."' AND r.pers_id = rp.pers_id AND rp.adr_id = ra.adr_id )"),0);
		if($nombre_de_responsable != 0)
		{
			$cpt_parents = '1';
			$requete_parents = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM ".$prefix_base."resp_pers rp, ".$prefix_base."resp_adr ra, ".$prefix_base."responsables2 r WHERE ( r.ele_id = '".$eleve_numid[$i]."' AND r.pers_id = rp.pers_id AND rp.adr_id = ra.adr_id ) ORDER BY resp_legal ASC");
