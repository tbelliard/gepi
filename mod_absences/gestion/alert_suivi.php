...
if ( $action_page != 'gestion_ag' ) {
	...
	$requete_alert_type = "SELECT * FROM ".$prefix_base."vs_alerts_types WHERE groupe_alert_type = '".$id_alert_groupe."'";
      	$resultat_alert_type = mysqli_query($GLOBALS["___mysqli_ston"], $requete_alert_type) or die('Erreur SQL !'.$requete_alert_type.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
...
// si on trouve on regarde dans la base des alerts_eleves s'il n'existe pas une alerte déjà enregistré pour cette élève
	      		$test_existance = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."vs_alerts_eleves WHERE eleve_alert_eleve = '".$donnee[0]."' AND groupe_alert_eleve = '".$id_alert_groupe."'"),0);
			if ( $test_existance === '0' )
			{
				...
				if ( isset($alert[$login]) ) { $alert[$login] = $alert[$login] + 1; } else { $alert[$login] = 1; }
					...
					// si oui on lit le total des trucs
			      		$requete_alert_eleve = "SELECT * FROM ".$prefix_base."vs_alerts_eleves WHERE ( eleve_alert_eleve = '".$donnee[0]."' AND groupe_alert_eleve = '".$id_alert_groupe."' ) ";
			               	$resultat_alert_eleve = mysqli_query($GLOBALS["___mysqli_ston"], $requete_alert_eleve) or die('Erreur SQL !'.$requete_alert_eleve.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
			...
			if ( isset($alert[$login]) )
			{
				if ( $alert[$login] === $nombre_de_type_passe)
				{
					...
					$requete_alert_eleve = "INSERT INTO ".$prefix_base."vs_alerts_eleves (eleve_alert_eleve, date_alert_eleve, groupe_alert_eleve, type_alert_eleve, nb_trouve, temp_insert) VALUES ('".$login."', '".$date_ce_jour_sql."', '".$id_alert_groupe."', '".$type_alert[$i_cpt]."', '".$nb_fois[$i_cpt]."', '".$temps_insert."')";
				        $execution_requete_alert_eleve = mysqli_query($GLOBALS["___mysqli_ston"], $requete_alert_eleve);
				}
			}
			$i_cpt = $i_cpt + 1;
		}
...
if ( ( $action_sql === 'nouveau_alert_groupe' or $action_sql === 'modifier_alert_groupe' ) and $valide_form === 'yes' ) {
	...
	if ( $action_sql === 'nouveau_alert_groupe' ) { $id_alert_groupe = ((is_null($___mysqli_res = mysqli_insert_id($GLOBALS["___mysqli_ston"]))) ? false : $___mysqli_res); }
	// vider les variables
...
if ( $action_sql === 'supprimer_alert_groupe' and $valide_form === 'yes' ) {
	...
	// on vérifie s'il existe une alerte
	      $test_existance = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."vs_alerts_groupes WHERE id_alert_groupe = '".$id_alert_groupe."'"),0);
		if ( $test_existance != '0' )
		{
		      ...
		      $test_existance = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."vs_alerts_types WHERE groupe_alert_type = '".$id_alert_groupe."'"),0);
			if ( $test_existance != '0' )
			{
		      ...
		      $test_existance = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."vs_alerts_eleves WHERE groupe_alert_eleve = '".$id_alert_groupe."'"),0);
			if ( $test_existance != '0' )
			{
	...
	// on vérifie s'il n'existe pas une alerte de même nom
		...
		if ( $test_existance != '0' )
		{
			$requete_alert_groupe = "SELECT * FROM ".$prefix_base."vs_alerts_groupes WHERE id_alert_groupe = '".$id_alert_groupe."'";
	               	$resultat_alert_groupe = mysqli_query($GLOBALS["___mysqli_ston"], $requete_alert_groupe) or die('Erreur SQL !'.$requete_alert_groupe.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
...
if ( $action_sql === 'supprimer_alert_type' and $valide_form === 'yes' ) {
	...
	// on vérifie s'il n'existe pas une alerte de même nom
	      $test_existance = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."vs_alerts_types WHERE id_alert_type = '".$id_alert_type."'"),0);
		if ( $test_existance != '0' )
		{
	...
	<?php if ( $editer_ce_groupe != 'non' ) { ?>
		...
		<optgroup label="Type action">
			...
			$requete_action ="SELECT * FROM ".$prefix_base."absences_actions ORDER BY init_absence_action ASC";
		        $execution_action = mysqli_query($GLOBALS["___mysqli_ston"], $requete_action) or die('Erreur SQL !'.$requete_action.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
