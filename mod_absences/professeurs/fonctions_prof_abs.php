...
function suivi_absence($creneau_id, $eleve_id){
	...
	if (getSettingValue("creneau_different") != 'n') {
		...
		$req = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id, retard_absence FROM absences_rb WHERE
								eleve_id = '".$eleve_id."'
...
function periode_actuel_jourdifferent($heure_choix) {
	...
	$resultat_periode = mysqli_query($GLOBALS["___mysqli_ston"], $requete_periode)
					or die('Erreur SQL !'.$requete_periode.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
