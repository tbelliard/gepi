   ...
   while ($total < $nb_ajout)
      {
            ...
            if($init_motif_absence_ins != "" && $def_motif_absence_ins != "")
            {
                 ...
                 if($action_sql == "modifier") { $test = mysql_result(mysql_query("SELECT count(*) FROM absences_motifs WHERE id_motif_absence != '".$id_motif_absence_ins."' AND init_motif_absence = '".$init_motif_absence_ins."'"),0); }
                 if ($test == "0")
                  {
...
if ($action_sql == "supprimer") {
      $test = mysql_result(mysql_query("SELECT count(*) FROM absences_motifs, absences_eleves
...
<?php if ($action=="modifier" OR $action=="ajouter") echo "<a href=\"admin_motifs_absences.php?action=visualiser\">Retour à la définition des motifs</a>"; ?>
<?php if ($action=="visualiser") echo "<a href=\"admin_motifs_absences.php?action=ajouter\">Ajouter un ou des motif(s)</a>"; ?>
|
