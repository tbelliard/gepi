   ...
   while ($total < $nb_ajout)
      {
	    ...
	    // vérification des champs non vide
            ...
            {
			$test = '1';
                            if($action_sql == "modifier") { $test = mysql_result(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) FROM ".$prefix_base."edt_calendrier WHERE id_calendrier = '$id_calendrier_ins'"),0); }
                              if ($test === '1')
                              {
...
if ($action == "modifier")
 {
      ...
      $resultat_modif_calendrier = mysqli_query($GLOBALS["___mysqli_ston"], $requete_modif_calendrier) or die('Erreur SQL !'.$requete_modif_calendrier.'<br />'.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
