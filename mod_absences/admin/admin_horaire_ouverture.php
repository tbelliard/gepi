...
//echo "<p>\$action_sql=$action_sql</p>\n";
...
if ( $action_sql === 'ajouter' or $action_sql === 'modifier' ) {
	...
	while ( $i < '7' ) {
		if( isset($ouvert[$i]) and !empty($ouvert[$i]) ) {
			...
			$requete="DELETE FROM ".$prefix_base."horaires_etablissement WHERE jour_horaire_etablissement = '".$tab_sem[$i]."' AND date_horaire_etablissement = '0000-00-00'";
			mysql_query($requete) or die('Erreur SQL !'.$requete.'<br />'.mysql_error());
		}
...
<form method="post" action="admin_horaire_ouverture.php?action=<?php echo $action; ?>" name="form1">
	...
	<tbody>
		...
		<?php
			...
			while ( $i < '7' ) {
				echo "<td>";
				echo "<input name=\"ouvert[$i]\" value=\"1\" type=\"checkbox\"";
				if ( isset($ouvert[$i]) and $ouvert[$i] === '1' ) {
		...
		<?php
			...
			while ( $i < '7' ) {
				echo "<td>";
				echo "<input name=\"ouverture[$i]\" size=\"5\" maxlength=\"5\"  value=\"";
				if ( isset($ouvert[$i]) and $ouvert[$i] === '1' ) {
					if ( isset($ouverture[$i]) and !empty($ouverture[$i]) ) {
						echo $ouverture[$i];
					}
				}
				echo "\" class=\"input_sans_bord\" />";
				$i = $i + 1;
				echo "</td>\n";
			}
		?>
		...
		<?php
			...
			while ( $i < '7' ) {
				echo "<td>";
				echo "<input name=\"fermeture[$i]\" size=\"5\" maxlength=\"5\"  value=\"";
				if ( isset($ouvert[$i]) and $ouvert[$i] === '1' ) {
					if ( isset($fermeture[$i]) and !empty($fermeture[$i]) ) {
						echo $fermeture[$i];
					}
				}
				echo "\" class=\"input_sans_bord\" />";
		...
		<?php
			...
			while ( $i < '7' ) {
				echo "<td>";
				echo "<input name=\"pause[$i]\" size=\"5\" maxlength=\"5\"  value=\"";
				if ( isset($ouvert[$i]) and $ouvert[$i] === '1' ) {
					if ( isset($pause[$i]) and !empty($pause[$i]) ) {
						echo $pause[$i];
					}
				}
				echo "\" class=\"input_sans_bord\" />";
		...
		<?php
			...
			while ( $i < '7' ) {
				echo "<td>";
				if ( isset($temps_total_ouverture[$i]) and !empty($temps_total_ouverture[$i]) ) {
					echo $temps_total_ouverture[$i];
				}
				echo "</td>\n";
				$i = $i + 1;
			}
