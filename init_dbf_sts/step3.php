...
if (isset($is_posted) and ($is_posted == "yes")) {
    $call_data = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT ID_TEMPO,ELENOM,ELEPRE,ELENOET,ERENO,ELESEXE,ELEDATNAIS,ELEDOUBL,ELENONAT,ELEREG,DIVCOD,ETOCOD_EP FROM temp_gep_import ORDER BY DIVCOD,ELENOM,ELEPRE");
    $nb = mysqli_num_rows($call_data);
    $i = "0";
    while ($i < $nb) {
        $req = mysqli_query($GLOBALS["___mysqli_ston"], "select col2 from tempo2 where col1 = '$i'");
        ...
        $maj_tempo = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE temp_gep_import SET LOGIN='$reg_login' WHERE ID_TEMPO='$id_tempo'");
        $reg_eleve = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO eleves SET no_gep='$no_gep',login='$reg_login',nom='$reg_nom',prenom='$reg_prenom',sexe='$reg_sexe',naissance='$reg_naissance',elenoet='$reg_elenoet',ereno='$reg_ereno'");
    ...
    $call_data = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT ELENOM,ELEPRE,ELENOET,ERENO,ELESEXE,ELEDATNAIS,ELEDOUBL,ELENONAT,ELEREG,DIVCOD,ETOCOD_EP FROM temp_gep_import ORDER BY DIVCOD,ELENOM,ELEPRE");
    $nb = mysqli_num_rows($call_data);
    $i = "0";
