...
// variables globales pour passage vues
global $liste_natures;
...
class CategoriesCtrl extends Controleur {
    ...
    function index() {
        ...
        $liste_categories=$this->modele_incidents->get_infos_categories();
        $liste_natures=$this->add_infos_categories($this->natures,$liste_categories);
        include('categories.php');
        echo"<script type='text/javascript'>initSortable2();</script>";
    }
    ...
    function save() {
        ...
        if(is_null($this->natures_selected)){ echo"<script type='text/javascript'>alert('Selectionnez des natures d\'incidents et une catégorie')</script>";}
        else{$this->categorie_selected=$_REQUEST['categorie'];
