...
Class Modele_Incidents extends Modele {
    ...
    public function get_incidents($tri=Null,$filtre_cat=Null,$filtre_mes=Null,$filtre_san=Null,$filtre_role=Null) {
        $this->get_db_incidents($tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
    ...
    public function get_nombre_total_mesures($liste_incidents=Null) {
        unset($this->nbre);
        $this->nbre=$this->get_db_nbre_total_mesures($liste_incidents);
        return($this->nbre[0]);
    }
    ...
    private function get_db_incidents($tri=Null,$filtre_cat=Null,$filtre_mes=Null,$filtre_san=Null,$filtre_role=Null) {
     //On selectionne dans tous les cas les incidents de l'étab pour les totaux
            $this->get_infos_incidents('etab_all','L\'Etablissement',Null,$tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
        ...
        if (isset($_SESSION['eleve_all'])) {
            $this->get_infos_incidents('eleves_all','Tous les élèves',Null,$tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
        ...
        if (isset($_SESSION['pers_all'])) {
            $this->get_infos_incidents('pers_all','Tous les personnels',Null,$tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
        ...
        if (isset($_SESSION['stats_classes_selected'])) {
            foreach($_SESSION['stats_classes_selected'] as $value) {
                ...
                foreach($this->liste_eleves as $this->classe) {
                    ...
                    $this->get_infos_incidents('classe',$this->infos_classe[0]['classe'],parent::make_list_for_request_in($this->classe),$tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
        ...
        if (isset($_SESSION['individus'])) {
            foreach ($_SESSION['individus'] as $value) {
                ...
                $this->get_infos_incidents('individu',$value[0],$value[0],$tri,$filtre_cat,$filtre_mes,$filtre_san,$filtre_role);
    ...
    private function get_infos_incidents($choix,$titre,$critere=Null,$tri=Null,$filtre_cat=Null,$filtre_mes=Null,$filtre_san=Null,$filtre_role=Null) {
        ...
        $this->sql.=' WHERE date BETWEEN \''.Gepi_Date::format_date_fr_iso($_SESSION['stats_periodes']['du']).
       ...
       if($filtre_san) {
        ...
        $this->res=mysql_query($this->sql);
        $this->incidents[$titre]=parent::set_array('object',$this->res);
    }
    ...
    private function get_db_nbre_total_mesures($liste=Null) {

        $this->sql='SELECT COUNT(str.id) from s_traitement_incident str LEFT JOIN s_incidents sin  ON
            str.id_incident=sin.id_incident WHERE sin.date BETWEEN \''.Gepi_Date::format_date_fr_iso($_SESSION['stats_periodes']['du']).
                '\' AND \''.Gepi_Date::format_date_fr_iso($_SESSION['stats_periodes']['au']).'\'  ';
