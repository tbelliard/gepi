	...
	setTimeout(\"test_stop('2')\",3000);
		...
		case 2:
					...
					for($i=0;$i<$nb_ele_id;$i++){
						...
						if($elenoet[$i]!='') {
							if((isset($ele_id[$i]))&&($ele_id[$i]!='')) {
								...
								fclose($fsql);
								*/
								$correction1=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
			...
			if(!$res=mysqli_query($GLOBALS["___mysqli_ston"], $sql)) {
				...
				if(my_eregi("Illegal mix of collations",((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)))) {
