...
function cherche_udt_ligne($nom_regroup) {
	...
	$res_udt_ligne=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
...
if ($recommencer != 'non' AND is_numeric($recommencer)) {
    ...
    $supprimer = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM edt_init WHERE ident_export >= '" . $recommencer . "' AND ident_export != 'fichierTexte2'")
    OR trigger_error('Erreur, La table edt_init n\'a pas été mise à jour : ' . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)), E_USER_ERROR);
    $modifier = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE edt_init SET nom_export = '" . $recommencer . "' WHERE ident_export = 'fichierTexte2'")
    OR trigger_error('Erreur dans le retour en arrière : ' . ((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)), E_USER_ERROR);
...
$query = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom_export, nom_gepi FROM edt_init WHERE ident_export = 'fichierTexte2'");
...
if ($query) {
    ...
    if ($etape == '') {
        $insert = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO edt_init SET ident_export = 'fichierTexte2', nom_export = '0', nom_gepi = '" . date("d-m-Y h:i") . "'");
        $etape = '0';
    }
    $aff_etape = '
        ...
        if (!$fp) {
            ...
            if ($etape != 12) {
                ...
                while ($tab = fgetcsv($fp, 1024, ";")) {
					...
					$sql="INSERT INTO tempo5 SET texte='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $toutelaligne) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""))."';";
