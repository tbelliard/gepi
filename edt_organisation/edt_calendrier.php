...
if (isset($data['calendrier']) AND isset($data['supprimer'])) {
	...
	$req_supp = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM edt_calendrier WHERE id_calendrier = '".$data['supprimer']."'") or Die ('Suppression impossible !');
    if ($data['supprimer'] != 0) {
        $req_supp_cours = mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM edt_cours WHERE id_calendrier = '".$data['supprimer']."'") or Die ('Suppression impossible !');
    }
...
if (isset($data['calendrier']) AND isset($data['copier_edt'])) {
    ...
    $req_edt_periode = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom_calendrier FROM edt_calendrier WHERE id_calendrier ='".$data['copier_edt']."'");
    ...
    if (PeriodExistsInDB($_SESSION['copier_periode_edt'])) {
        if (PeriodExistsInDB($data['coller_edt'])) {
            if ($data['coller_edt'] != $_SESSION['copier_periode_edt']) {
                $req_edt_periode = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM edt_cours WHERE 
                ...
                while ($rep_edt_periode = mysqli_fetch_array($req_edt_periode)) {
                    $sql = "SELECT id_cours FROM edt_cours WHERE 
                            ...
                            ";
                    $verif_existence = mysqli_query($GLOBALS["___mysqli_ston"], $sql) OR DIE('Erreur dans la vérification du cours : '.((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                    if (mysqli_num_rows($verif_existence) == 0) {
				        $nouveau_cours = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO edt_cours SET 
...
if (isset($data['new_periode']) AND isset($data['nom_periode'])) {
	...
	if ($formatdatevalid) {
		...
		$req_verif_periode = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom_calendrier FROM edt_calendrier WHERE nom_calendrier = '".$data['nom_periode']."'"));
		if ($req_verif_periode[0] == NULL) {
			...
			if ($data['nom_periode'] != '' AND $heuredeb_ts != '' AND $heurefin_ts != '') {
				$req_insert = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO edt_calendrier (`nom_calendrier`, `classe_concerne_calendrier`, `debut_calendrier_ts`, `fin_calendrier_ts`, `jourdebut_calendrier`, `heuredebut_calendrier`, `jourfin_calendrier`, `heurefin_calendrier`, `numero_periode`, `etabferme_calendrier`, `etabvacances_calendrier`)
...
if (isset($data['modif_ok']) AND isset($data['nom_periode'])) {
	...
	$modif_periode = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE edt_calendrier
...
$data['req_affcalendar'] = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM edt_calendrier ORDER BY jourdebut_calendrier") OR die ('Impossible d\'afficher le calendrier.');
...
for ($i=0; $i<$data['nbre_affcalendar']; $i++) {
	...
	if ($data['expl_aff'][$i] == "0" OR $data['rep_affcalendar'][$i]["classe_concerne_calendrier"] == "0") {
		...
		for ($t=0; $t<(count($data['expl_aff'][$i]) - 1); $t++) {
			$req_nomclasse = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom_complet FROM classes WHERE id = '".$data["expl_aff"][$i][$t]."'"));
...
if (isset($data['calendrier']) AND isset($data['modifier'])) {
	...
	$rep_modif = mysqli_fetch_array(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM edt_calendrier WHERE id_calendrier = '".$data['modifier']."'"));
	...
	*/
	...
	$toutes_classes = explode(";", $rep_modif["classe_concerne_calendrier"]);
		...
		*/
		...
		$res_clas_max_per=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
		$id_classe_max_per=mysql_result($res_clas_max_per,0,"id_classe");
		$req_periodes = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT nom_periode, num_periode FROM periodes WHERE id_classe = '$id_classe_max_per'");
	...
	if(mysqli_num_rows($res)>0) {
		while($lig=mysqli_fetch_object($res)) {
			...
			for($loop=0;$loop<count($tab_classe);$loop++) {
				$register=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE periodes SET date_fin='".$lig->jourfin_calendrier."' WHERE (num_periode='".$lig->numero_periode."' and id_classe='".$tab_classe[$loop]."')");
				if(!$register) {
