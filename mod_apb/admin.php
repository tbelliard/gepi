...
if (isset($_POST['posted_selection'])) {
	check_token();
	$errors = false;
	$req_classes = mysqli_query($GLOBALS["mysqli"], 'SELECT id,classe,nom_complet,apb_niveau FROM classes ORDER BY classe');
	while ($classe = mysqli_fetch_object($req_classes)) {
		...
		if ($classe->apb_niveau != $new_value) {
			$rec_classe = mysqli_query($GLOBALS["mysqli"], "UPDATE classes SET apb_niveau = '".$new_value."' WHERE id = '".$classe->id."'");
			if (!$rec_classe) {
				$errors = true;
...
$req_classes = mysqli_query($GLOBALS["mysqli"], 'SELECT id,classe,nom_complet,apb_niveau
								  FROM classes
								  ORDER BY classe');
