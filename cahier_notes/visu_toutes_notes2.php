...
while($j < $nb_lignes_tableau) {
    ...
    if ($aff_date_naiss){
		...
		if(strlen($tmptab[0])==4){$tmptab[0]=mb_substr($tmptab[0],2,2);}
...
*/
	...
	$nom_complet_coupe = (strlen($nom_complet_matiere) > 20)? urlencode(mb_substr($nom_complet_matiere,0,20)."...") : urlencode($nom_complet_matiere);

	$nom_complet_coupe_csv=(strlen($nom_complet_matiere) > 20) ? mb_substr($nom_complet_matiere,0,20) : $nom_complet_matiere;
