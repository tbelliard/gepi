...
if ($id_ct != null) {
	...
	if ($groupe == null) {
		echo("Erreur : pas de groupe ou mauvais groupe sp�cifi�");
		die;
	}
...
if ($ctCompteRendu->getVise() == 'y') {
	echo("Erreur : Notice sign�e, edition impossible/");
	die();
}
...
if (!empty($doc_name_modif) && (trim($doc_name_modif)) != '' && !empty($id_document)) {
	...
	if (empty($documents)) {
		echo "Erreur : document non trouv�.";
		die();
	}
