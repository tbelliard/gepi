...
function getProfsEPI($idGroupeEPI) {
	 ...
	 */
	/*
	$sqlProfDiscipline_2 = "SELECT DISTINCT t0.*, jpm.id_matiere FROM ("
		. "SELECT jau.* , SUBSTR(u.numind, 2) AS numind "
		. "FROM j_aid_utilisateurs as jau "
		. "INNER JOIN utilisateurs AS u "
		. "ON u.login = jau.id_utilisateur "
		. "WHERE jau.id_aid =  $idGroupeEPI "
		. ") AS t0 "
		. "INNER JOIN "
		. "j_professeurs_matieres AS jpm "
		. "ON jpm.id_professeur = t0.id_utilisateur ";
	
	$sqlProfDiscipline_3 = "SELECT t1.*, t2.id_utilisateur AS prof1 FROM ($sqlProfDiscipline_1) AS t1 "
		. "INNER JOIN "
		. "($sqlProfDiscipline_2) AS t2 "
		. "ON t1.matiere1 =  t2.id_matiere  ";
	
	$sqlProfDiscipline_4 = "SELECT t3.*, t4.id_utilisateur AS prof2 FROM ($sqlProfDiscipline_3) AS t3 "
		. "INNER JOIN "
		. "($sqlProfDiscipline_2) AS t4 "
		. "ON t3.matiere2 = t4.id_matiere ";";
	 * 
	 */
	
	
	$sqlProfDiscipline_1 = "SELECT t0.* FROM ("
		. "(SELECT a.id, a.nom, a.matiere1 AS matiere FROM aid AS a) "
		. "UNION "
		. "(SELECT a.id, a.nom, a.matiere2 AS matiere FROM aid AS a)"
		. ") AS t0 WHERE t0.id = $idGroupeEPI ";
	
	$sqlProfDiscipline_2 = "SELECT jau.*, substr(u.numind,2) AS numind FROM j_aid_utilisateurs AS jau "
		. "INNER JOIN utilisateurs AS u "
		. "ON u.login = jau.id_utilisateur "
		. "WHERE id_aid = $idGroupeEPI ";
	
	
	$sqlProfDiscipline_3  = "SELECT DISTINCT t1.*, t2.id_matiere FROM ("
		. "$sqlProfDiscipline_2"
		. ") AS t1 "
		. "INNER JOIN "
		. "(SELECT * FROM j_professeurs_matieres AS jpm) AS t2 "
		. "ON t2.id_professeur = t1.id_utilisateur ";
	
	$sqlProfDiscipline_4 = "SELECT t4.id_aid , t3.nom , t3.matiere , t4.id_utilisateur, t4.indice_aid, t4.numind FROM ($sqlProfDiscipline_1) AS t3 "
		. "INNER JOIN"
		. "($sqlProfDiscipline_3) AS t4 "
		. "ON t4.id_matiere = t3.matiere";
	
	
		
	//echo  $sqlProfDiscipline_4."<br><br>";
